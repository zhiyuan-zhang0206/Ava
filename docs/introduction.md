...
traces, logs, and metrics in one UI through the authenticated gateway
`/grafana/` path; every backend port, including Grafana 3003, is loopback-only.
It also backs the gateway's /ops + inspect endpoints:
