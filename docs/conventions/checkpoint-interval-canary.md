...
Operational protocol for rolling `AVA_CHECKPOINT_INTERVAL > 1` out to the fleet
one agent subset at a time — the formalized version of the enablement notes in
...
`N=4` cuts checkpoint write volume by ~75%).

## When to use this protocol

`AVA_CHECKPOINT_INTERVAL` is a per-agent flag (`per_agent=True`,
`restart_required=agent`), default `1` = write every super-step (zero behavior
change). Raising it trades crash-recovery granularity for write volume; there
is **no auto-rollout** — every N>1 deployment goes through this protocol.
...
1. **Pick the canary subset.** Start with 2-3 low-traffic, non-critical
   ...
   per-agent overlay `{"checkpoint_interval": 4}` (config overlay wins over
   cluster `.env`; do NOT set the cluster-wide `.env` value first).
...
4. **Forced kill → resurrect recovery check** (the thing that could regress):
   ...
   - the checkpoint parent chain is walkable (no broken ancestors in the
     fleet view).
   Do this at N=4 before widening; if recovery is broken, roll back (below).
5. **Widen**: canary a second subset covering one resident role + one
   ...
   consider a cluster `.env` `AVA_CHECKPOINT_INTERVAL=4` default (still
   per-agent overridable).

## Rollback

- **Per-agent**: clear the overlay (or set `checkpoint_interval` back to 1)
  and restart the agent. The next checkpoint writes at the old cadence; a
  crash during the transition replays at most the current interval's N-1
  steps — the same bound as normal N-step operation, no special recovery.
- **Cluster-wide**: remove `AVA_CHECKPOINT_INTERVAL` from `.env` and restart
  the affected agents. There is no data migration — the flag only changes
  write cadence; existing checkpoints (sparser parent chains included) remain
  valid and readable at any interval.
...
- One full day at N=4 on the canaries with write-volume reduction observed.
