...
| Boot autostart | launchd / systemd unit (crontab `@reboot` fallback, `shared/os_boot_unit.py`) | a logon task, registered automatically by the converge phase on every `ava start` — nothing manual. It runs `ava boot` (retry loop) rather than `ava start`: `schtasks /RI` does not apply to a logon trigger, so the retry cannot live in the scheduler. **Logon, not machine start** — see [Reboot](#a-reboot-with-no-logon-does-not-bring-ava-back) below |
