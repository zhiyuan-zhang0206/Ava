...
native side ends it (restart/stop) or its TTL expires. The attestation anchor set is the supported controller ends — currently `codex` and `claude` (`shared/_impersonation_store.verify_caller`); extending support to a new controller CLI means extending that set and this list together.
