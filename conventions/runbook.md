...
- **A health probe never reloads its own launchd label.** Auto-rollback runs
  ...
  terminate that whole recovery process tree. Registration defers when it can
  prove it runs inside the job's live process tree (`launchctl print` pid +
  `ps` ancestry walk) — the inherited `XPC_SERVICE_NAME` alone is not proof
  (descendants read "0"; `postmortems/0008`) — leaves the existing plist
  untouched, and lets the next external converge apply any pending spec change.
...
**file-backed sending queues** (file_storage), so their accepted backlog
survives sidecar restarts; metrics use a 1,000-request in-memory queue. Every
queue rejects the newest batch immediately when full, and the collector's
