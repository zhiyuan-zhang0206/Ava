...
| `idle-shell-reminder` (gateway only) | `.venv/bin/python -m services.idle_shell_reminder.daemon` (every 60s polls this machine's persistent PTY shells; after 5m idle it sends the owner a merged chat reminder, then backs off through 30m/1h/2h/4h/8h/16h to a 24h cadence; an owner reply containing `保留` makes every shell covered by that reminder exempt until it ends; never closes a shell; gate with `AVA_IDLE_SHELL_REMINDER_ENABLED`) | `services.healthchecks.idle_shell_reminder` (`/healthz` :8115) |
