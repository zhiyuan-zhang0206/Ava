...
| `contributions.*` | Declared contribution surfaces (VS Code `contributes` analog). **The declaration is documentation; registration is fact.** A load-time diff between the two (declared-but-not-registered = warning, registered-but-not-declared = fail-fast) lands with the runtime work (S3), reusing the drift-detection pattern of `agent/hooks/capabilities.py` | surfaces pre-checkable, listable, auditable |
