...
- `lint_code_structure.py` + `structure/quality_budget.py` + `structure/locality.py` — 800-line/20-entry/CC/nesting budgets plus locality (package doors, single decision owners; see `python-conventions.md`). `structure/baseline.json` also freezes `private_imports` / `owner_bypasses` as exact `path::target -> site count` maps — growth or shrinkage both fail until edited. CC 10–14 warns; `--complexity-warnings-full` unfolds counts.
