...
def prove_publication_input(home: Path, receipt: Path) -> None:  # noqa: PLR0915 — isolated selector/receipt lifetime, always restored.
    ...
    chosen = {
        ...
        "prepared_receipt_digest": hashlib.sha256(original_receipt).hexdigest(),
    }
    try:
        ...
        if (
            ...
            or actual.actual.inventory_digest != json.loads(original_receipt)["inventory_digest"]
            or actual.actual.prepared_receipt_digest != chosen["prepared_receipt_digest"]
        ):
        ...
        for mutation in ("selector", "receipt", "home"):
            ...
            if mutation == "selector":
                ...
                selector.write_text(json.dumps(chosen | {"prepared_receipt_digest": digest}))
            try:
                try:
                    revalidate_runtime_publication_input(actual)
                except ReleaseRejectedError:
