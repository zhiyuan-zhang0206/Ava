...
def retained_journal() -> dict[str, object] | None:
    recovery = updater_handoff.read_bootstrap_recovery()
    if recovery is None:
        return None
    journal = recovery["journal"]
    if not isinstance(journal, dict):
        raise TypeError("retained bootstrap journal is not an object")
...
def failure_snapshot(home: Path, old: hop.ExpectedProcess) -> str:
    ...
    try:
        ...
        evidence["journal_stage"] = (
            journal.get("stage", "not-created") if journal else "not-created"
        )
    except FileNotFoundError:
        ...
        projection = ObserverProjection.model_validate(
            {
                ...
                "ops_port": port,
                "transport_encryption": env["AVA_TRANSPORT_ENCRYPTION"],
            }
        )
        ...
        try:
            for mode in (
                ...
                try:
                    ...
                    journal = retained_journal()
                    stage = (
                        journal.get("stage", "before-bootstrap-journal")
                        if journal
                        else "before-bootstrap-journal"
                    )
                    stderr = exc.stderr or b""
                ...
                if mode == "crash-after-stop":
                    require(
                        (retained_journal() or {}).get("stage") == "old_stopped",
