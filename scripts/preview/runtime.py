...
def require_success(response: httpx.Response) -> None:
    """Keep the server's failure reason in the recorded command traceback."""
    try:
        response.raise_for_status()
    except httpx.HTTPStatusError as exc:
        exc.add_note(f"Response body (first 4096 characters): {response.text[:4096]}")
        raise
...
def check(run: Path, home: Path) -> None:
    ...
    if failures:
        raise RuntimeError(f"Services not ready after successful start: {failures}")
...
def verify_stopped(run: Path, home: Path) -> None:
    """Verify completed destroy, including retired source binding; not ordinary stop."""
        ...
        json.dumps(
            {
                ...
                "checkout_binding_released": True,
            }
        )
        + "\n"
    )
