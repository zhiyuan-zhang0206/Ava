...
_ALLOWED_FILES = frozenset(
    {
        ...
        "shared/platform.py",  # ensure_utf8_stdio sets PYTHONUTF8/PYTHONIOENCODING in os.environ so spawned child interpreters start in UTF-8 mode (Windows cp1252 console fix); Python runtime knobs, not Ava Settings fields
        ...
        "ava/_mcp_remote.py",  # _current_agent_id() reads the ambient AVA_AGENT_ID to stamp MCP daemon envelopes; the key is the process identity channel, not Settings-managed, and importing ava.self here is circular (moved from ava/mcps.py, 2026-08-13 #1229)
        ...
        "shared/lm/_providers.py",  # _grok_conv_id() reads AVA_AGENT_ID (per-process agent identity set by agent/loop.py) for the grok cache-affinity header; Settings is a module-load singleton with no per-agent identity field
