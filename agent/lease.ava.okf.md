...
- `ALIVE_STATUSES = (starting, running, idling)` — `starting` is alive (process up, booting); `hibernating` is deliberately excluded (swapped out, no process, reaper-exempt).
...
- **Claim**: `agent/_starting.py:enter_starting_state` writes `lease_expires_at = now() + AGENT_LEASE_TTL_S` (600 s) in the same UPDATE as `status='starting'` — every spawn / resurrect / respawn path converges here, so a row is never alive without a lease.
...
- **Clear** (hygiene — the lease dies with the process): exit (`ops/ops_lifecycle.py:_mark_exited_blocking`), hibernation (`_mark_hibernating_blocking`), resurrect / respawn / swap-in (`ops/agent_wake.py` — the row returns to `allocated` with no lease; the next claim re-grants), and integrity-terminate all drop `lease_expires_at`.

### Observers

- **Reaper** — `ops/controllers/respawn.py:_collect_local_lease_zombies`: `running`/`idling` rows with an expired or NULL lease are zombies; a resident pid behind an expired lease is force-killed (wedged or pre-lease code), then the revive pass relaunches the row in place. `hibernating` exempt by design. The pass holds a post-outage grace window (`AGENT_LEASE_ZOMBIE_GRACE_S` = 180 s) after the restarter's own DB link recovers, so a paused-but-alive agent whose lease expired during the outage gets time to renew instead of being killed (2026-08-08 audit P1-2).
- **Wedged / hibernate selectors** require an unexpired lease — lease-expired rows are the reaper's domain; lease-live-but-stuck rows stay wedged's.
- **Heartbeat daemon** (`services/heartbeat/daemon.py`): an *idling* nudge target must hold an unexpired lease (an expired idling row is a zombie — nudging it keeps a corpse busy); *hibernating* stays nudgeable with no lease (no process by design — the nudge is how it wakes).
...
- `agent/_starting.py:enter_starting_state` — lease birth (claim)
