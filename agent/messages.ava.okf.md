...
- **Attachment delivery** (`attach_message`): appends one HumanMessage whose first block is the attachment caption and whose remaining blocks are model-native media. It carries `ava_msg_type="attach"` and a creation time, without a serving URL.
