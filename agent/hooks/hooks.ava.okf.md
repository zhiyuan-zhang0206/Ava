...
- **Activation telemetry**: a hook registered by a *plugin* that returns a non-empty dict also emits one `plugin_activation` event naming the keys it wrote (`shared/plugin_activation.py`; attribution captured at `_register` into the weak-keyed `_HOOK_PLUGIN`). Framework hooks and `None` returns record nothing, so pure observation stays free. This is the runtime half of the registration ledger and philosophy §6's obsolescence gauge — see [[okf/plugins/plugins.ava.okf.md]]
...
- Plugin hooks are also `Hook` subclasses (e.g., `ava_builtins/plugins/ava_sdk_reminder/plugin.py:_SdkReminderAfterExecHook`, `ava_builtins/plugins/ava_code/plugin.py:_InjectCwdNotesAfterExecHook`, `plugins/ava_memory`'s recall hook, etc.), written in the same style as built-in hooks—plugin authors don't need to care about built-in vs plugin distinction
