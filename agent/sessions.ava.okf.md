...
Every session has a **record** (`$AVA_HOME/run/sessions/<name>.json`: pid + start time) and a **log** (`$AVA_HOME/logs/<name>.out.log`). `ava cluster status` enumerates the same sessions; raw session output is queried in Loki (the local LGTM viewer — LogQL via Grafana Explore / logcli / the Loki HTTP API, label `service` = session name; see `deploy/local/lgtm/README.md`).
