...
- Immediately before agent-authored code begins, the child emits the noise-tier `exec_child_boot` telemetry event. Its `duration_ms` covers child runtime setup after initial module imports; the parent-owned execution duration still covers the full process lifecycle
