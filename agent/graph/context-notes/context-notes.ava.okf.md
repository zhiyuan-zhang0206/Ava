...
- Framework notes: agent id (`on_fork`), exec timeout, cluster timezone, preloaded skills (`on_fork` since issue #1320). Plugins append at load; `clear_plugin_registrations` truncates to `_FRAMEWORK_NOTE_COUNT`. `on_fork` = the subset grafted onto a fork's inherited history (`fork_notes`).
