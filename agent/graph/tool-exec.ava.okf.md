...
- stdout+stderr are combined into the same `StreamingTextIO` (`_exec_stream.py`), preserving order; streamed to Redis in real-time, finally returned to the LLM. The capture is **per context**, not a global redirect (`_exec_capture.py`): `sys.stdout`/`sys.stderr` hold stable routers that resolve to whatever capture the *calling* context bound, else the real stream. So concurrent execs in one process cannot cross-capture, and an orphaned worker's abandoned binding stays in its own context instead of leaving the process pointed at a fileno-less buffer
