...
- **claim** (`_claim.py` + `_attach_drain.py` / `_claim_batch.py` / `_claim_routing.py` / `_claim_dispatch.py` / `_claim_decide.py`): At an idle turn boundary, drains pending attachments into one tail HumanMessage before taking the existing end-turn or wait route; otherwise long-waits on Redis pub/sub and dispatches inbound messages (new message / resume / terminate / restart). Chat dispatch can prepend one opt-in semantic skill-loading note before the untouched inbound message; matching is fail-open and disabled by default
