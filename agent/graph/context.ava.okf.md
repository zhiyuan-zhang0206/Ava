...
- **Dependency aggregation (handles)**: `ops_pool` (DB connection pool), `inbound_listener` (dedicated Redis pub/sub connection), `llm` (ChatModel instance), `redis_sync`, `event_publisher`; plus configuration snapshots `db_url` / `redis_url` / `events_channel` / `ava_home`. The MCP daemon is not aggregated here—it is a local object in `loop.py:main()`.
