...
```
- `idling (unclaimed) → terminated`: boot failure (schema gate, `agent/_starting.py:_mark_preclaim_terminated`, guarded by `status='idling' AND pid IS NULL`) or a launch that never confirmed (`ops/agent_launch.py`, both the off-path confirm and the retries-exhausted write, same predicate). **Every pre-claim termination reasserts the unclaimed predicate** — the row is contested by the child's own claim, and an unguarded terminate buries a live agent, then crash-resurrect launches a duplicate for it.
