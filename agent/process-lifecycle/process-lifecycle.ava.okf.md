...
```
- `idling (unclaimed) → terminated`: legacy boot rejection (`agent/_starting.py:_mark_preclaim_terminated`) requires absent runtime ownership and lifecycle command, in addition to `status='idling' AND pid IS NULL`. Owned attempts leave settlement to the fixed-command controller. Legacy launcher confirmation paths still use the unclaimed predicate and require separate ownership-fence audit before activation.
