...
- **Instruction dispatch**: `ava.help(ava.skills.<path>)` loads a skill body by path; nested paths like `ava.skills.web_ai.deep_research`. Directories and every displayed name are dash-separated (ecosystem-standard), and the display identifier joins namespace segments with `:` (`ava.skills.web-ai:deep-research`); underscore is only the Python projection, folded by `shared/packages/skills/skill_names.py` — see [[ava/skills.ava.okf.md|Skill System]] "Naming".
