...
Each concept file is written by an agent after reading the corresponding source code — not automatically generated by scripts. The graph is built **on demand by the gateway**: `GET /api/okf/graph` (`gateway/routers/okf_graph.py`) calls `shared/docs/okf_graph.py:build_graph_data()` per request and renders the D3 visualization from `scripts/okf-d3-template.html`. `scripts/build_okf_data.py` is a CLI wrapper over the same builder (for offline builds / viz serving); there is no checked-in `graph_data.json`.
