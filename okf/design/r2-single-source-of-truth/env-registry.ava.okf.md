...
Every env key is declared exactly once: Settings fields declare metadata at the class declaration (`json_schema_extra`); passthrough keys (PATH/TMPDIR/VIRTUAL_ENV/DISPLAY/HOME/USER/LOGNAME, the machine's network proxy configuration, Windows system keys) are registered rows. The registry builds from pydantic **class metadata only** (no Settings instantiation — decoupled from `dotenv_boot` timing). Three operation families become **projections** of the registry:
