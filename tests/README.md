...
`-m "not flaky"`, e2e `-n 2`) with `--store-durations --clean-durations`,
merges the two runs, keeps entries `>= 0.2s`, and rewrites `.test_durations`
in the committed compact-JSON format (sorted keys, 3-decimal values, one
trailing newline). It never edits the file in place: a failed backend run
aborts without touching it; a failed e2e run still keeps whatever durations it
recorded, and if it recorded nothing the previous e2e entries are retained.
