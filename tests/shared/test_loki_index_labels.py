...
def test_every_window_reads_as_one_indexed_slice() -> None:
    cutover = labels.INDEX_LABEL_CUTOVER_AT
    start = cutover - timedelta(days=1)
    end = cutover + timedelta(days=1)
    assert labels.split_index_label_window(start, end) == (
        labels.LokiReadSlice(labels.LokiReadEra.INDEXED, start, end),
    )
    assert labels.split_index_label_window(start, start) == ()
