...
@pytest.fixture()
def job(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):  # type: ignore[no-untyped-def]
    from shared import os_cron, os_logs_job
...
def test_launchd_plist_runs_rotate_then_retention_daily(job) -> None:  # type: ignore[no-untyped-def]
    ...
    assert "'/work tree/.venv/bin/ava' logs rotate" in values[2]
    assert "&& '/work tree/.venv/bin/ava' logs retention" in values[2]
    assert f"--family-days {job.FAMILY_DAYS}" in values[2]
...
def test_macos_reregistration_rewrites_and_reloads_idempotently(
    job, monkeypatch: pytest.MonkeyPatch
) -> None:  # type: ignore[no-untyped-def]
    calls: list[list[str]] = []
    monkeypatch.setattr(job.subprocess, "run", lambda argv, **_kwargs: calls.append(argv) or _ok())
...
def test_linux_registration_replaces_only_this_clusters_line(
    ...
    monkeypatch.setattr(job.shutil, "which", lambda _name: "/usr/bin/crontab")
...
def test_converge_registers_then_reaps_legacy_job(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from shared import os_logs_job

    calls: list[str] = []
    monkeypatch.setattr(os_logs_job, "register_logs_job", lambda: calls.append("register"))
    monkeypatch.setattr(os_logs_job, "reap_legacy_logs_job", lambda: calls.append("reap"))
