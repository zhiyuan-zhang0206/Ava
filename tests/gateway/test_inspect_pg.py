...
def test_inspect_values_whole_life_prefers_rollup_over_raw_scan(
    ...
    agent_id = _insert_agent(db_conn)
    # The PG events archive is gone since the #1823 cleanup; the raw archive
    # scan it once fed reads the Loki archive stream (asserted below to not
    # run at all when the rollup exists).
    with db_conn.cursor() as cur:
        cur.execute(
