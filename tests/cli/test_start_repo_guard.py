...
def test_dev_checkout_cannot_initialize_default_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    user = tmp_path / "user"
...
def test_unanchored_checkout_does_not_select_production(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_conflicting_pointer_is_not_overwritten(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
