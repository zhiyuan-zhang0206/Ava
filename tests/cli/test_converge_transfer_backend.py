...
def test_skips_on_single_box(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """A single box (also carries gateway) has no peer to transfer to — skip the probe."""
    import shared.google_drive as gd
...
def test_skips_when_backend_none(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
