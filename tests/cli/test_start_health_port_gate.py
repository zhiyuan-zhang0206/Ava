...
from tests.cli.test_start_readiness_gate import (
    _hermetic_start as _base_start,  # noqa: F401 — shared fixture
)
...
def _roster(monkeypatch: pytest.MonkeyPatch, specs: tuple[ServiceSpec, ...]) -> list[str]:
    ...
    monkeypatch.setattr(
        ...
        lambda _r: tuple((s, None) for s in specs),
    )
    monkeypatch.setattr(
        _cli, "_start_roster", lambda _roles, skip: tuple(s for s in specs if s.session not in skip)
    )

    def launch(roster, *_a, **_kw):
