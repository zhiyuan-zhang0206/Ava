...
def test_gateway_refresh_helper_never_raises(
    ...
    monkeypatch.setattr(_up, "subprocess", type("F", (), {"run": lambda *_a, **_kw: _R1()})())  # pyright: ignore[reportUnknownArgumentType]
    ...
    monkeypatch.setattr(_up, "subprocess", type("F", (), {"run": lambda *_a, **_kw: _R2()})())  # pyright: ignore[reportUnknownArgumentType]
    _local._refresh_builtin_skills(repo)
    ...
    monkeypatch.setattr(_up, "subprocess", type("F", (), {"run": staticmethod(_boom)})())  # pyright: ignore[reportUnknownArgumentType]
    _local._refresh_builtin_skills(repo)
...
def test_runner_leg_refreshes_after_checkout(monkeypatch: pytest.MonkeyPatch, repo: Path) -> None:
    ...
    monkeypatch.setattr(_cli, "_preflight_probes", lambda: 0)  # pyright: ignore[reportUnknownArgumentType, reportUnknownLambdaType]
    monkeypatch.setattr(_cli, "_do_stop", lambda *_a, **_kw: None)  # pyright: ignore[reportUnknownArgumentType, reportUnknownLambdaType]
...
def test_runner_refresh_helper_never_raises(
    ...
    monkeypatch.setattr(_runner, "subprocess", type("F", (), {"run": lambda *_a, **_kw: _R1()})())  # pyright: ignore[reportUnknownArgumentType]
    ...
    monkeypatch.setattr(_runner, "subprocess", type("F", (), {"run": staticmethod(_boom)})())  # pyright: ignore[reportUnknownArgumentType]
    _runner._refresh_builtin_skills(repo, repo / "ava")
    assert "non-fatal" in capsys.readouterr().err  # pyright: ignore[reportUnknownMemberType]
