...
_DEFAULT_MODEL_DEEPSEEK_FLASH_MIGRATION = (
    Path(__file__).resolve().parents[2]
    / "migrations"
    / "20260917T045040_default-model-deepseek-flash.sql"
)
...
def test_default_model_deepseek_flash_migration_updates_only_prior_seed(
    db_conn: psycopg.Connection, cluster_defaults_unset: None
) -> None:
    """Advance the prior migration-owned flash default to the renamed id, never an API choice."""
    up = _DEFAULT_MODEL_DEEPSEEK_FLASH_MIGRATION.read_text()
    down = _DEFAULT_MODEL_DEEPSEEK_FLASH_MIGRATION.with_suffix(".down.sql").read_text()

    with db_conn.cursor() as cur:
        cur.execute(
            "UPDATE cluster_defaults SET llm_model = 'deepseek-v4-flash', "
            "updated_by = 'migration' WHERE id = 1"
        )
        cur.execute(sql.SQL(cast(LiteralString, up)), prepare=False)
        cur.execute("SELECT llm_model, updated_by FROM cluster_defaults WHERE id = 1")
        assert cur.fetchone() == ("deepseek-flash", "migration")
