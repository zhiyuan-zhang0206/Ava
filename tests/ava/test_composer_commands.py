"""Composer command discovery + expansion (`ava.composer_commands`)."""

from pathlib import Path

import pytest

from ava import composer_commands
...
def test_scan_dir_parses_frontmatter(tmp_path: Path):
    ...
    assert composer_commands._scan_dir(tmp_path, ())["recap"] == {
        "name": "recap",
        "description": "recap it",
...
def test_scan_dir_reads_claude_code_argument_hint(tmp_path: Path):
    ...
    assert composer_commands._scan_dir(tmp_path, ())["deploy"]["instruction_hint"] == "env"


def test_scan_dir_no_frontmatter_uses_whole_body(tmp_path: Path):
    _write(tmp_path, "plain.md", "just a prompt, no frontmatter")
    out = composer_commands._scan_dir(tmp_path, ())["plain"]
...
def test_discover_dedups_later_source_wins(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(composer_commands, "_command_dirs", lambda: [(early, ()), (late, ())])  # pyright: ignore[reportUnknownArgumentType]
...
def test_discover_sorted_by_name(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(composer_commands, "_skill_commands", dict)
    assert [c["name"] for c in composer_commands.discover_commands()] == ["alpha", "zebra"]
...
def test_expand_passes_through_non_command():
    assert composer_commands.expand_command("just chatting") == "just chatting"
    assert composer_commands.expand_command("/") == "/"


def test_expand_passes_through_unknown_command(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(composer_commands, "discover_commands", list)
    assert composer_commands.expand_command("/nope do x") == "/nope do x"
...
def test_expand_known_command_without_arg(monkeypatch: pytest.MonkeyPatch):
    ...
    assert composer_commands.expand_command("/recap") == "Command /recap:\nRecap it."
...
def test_expand_known_command_with_arg(monkeypatch: pytest.MonkeyPatch):
    ...
    assert composer_commands.expand_command("/recap just the PRs") == (
...
def test_expand_passthrough_when_disabled(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(composer_commands.settings.agent, "commands_enabled", False)
    assert composer_commands.expand_command("/recap just the PRs") == "/recap just the PRs"
...
def _chain_catalog(monkeypatch: pytest.MonkeyPatch, *commands: dict) -> None:
    monkeypatch.setattr(composer_commands, "discover_commands", lambda: list(commands))  # pyright: ignore[reportUnknownArgumentType]
...
def test_chain_mixes_file_command_and_skill_as_command(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        ...
        "discover_commands",
        lambda: [
            _file_command("recap", "Recap it."),
            *composer_commands._skill_commands().values(),
        ],
    )
    out = composer_commands.expand_command("/recap the week /goal")
...
def test_slash_token_that_is_not_a_command_stays_in_the_argument(monkeypatch: pytest.MonkeyPatch):
    ...
    _chain_catalog(monkeypatch, _file_command("recap", "Recap it."))
    assert composer_commands.expand_command("/recap check /path/to/file") == (
...
def test_text_not_starting_with_a_command_never_chains(monkeypatch: pytest.MonkeyPatch):
    ...
    assert composer_commands.expand_command("check /recap output") == "check /recap output"


def test_unknown_leading_name_passes_the_whole_chain_through(monkeypatch: pytest.MonkeyPatch):
    _chain_catalog(monkeypatch, _file_command("recap", "Recap it."))
    assert composer_commands.expand_command("/nope /recap") == "/nope /recap"
...
def test_lifecycle_command_chains_like_any_other(monkeypatch: pytest.MonkeyPatch):
    ...
    assert composer_commands.expand_command("/recap /compact").index("Command /recap:") < (
        composer_commands.expand_command("/recap /compact").index("Command /compact:")
    )
...
def test_repeating_one_command_expands_it_twice(monkeypatch: pytest.MonkeyPatch):
    ...
    assert composer_commands.expand_command("/compact /compact") == (
        "Command /compact:\nWind down.\n\nCommand /compact:\nWind down."
    )
...
def test_command_record_carries_no_chaining_policy():
    ...
    fields = set(composer_commands.Command.__annotations__)
