"""ava.agent_identity — the one-input identity bootstrap and the owns_loop guard.
...
def test_require_agent_id_returns_when_set() -> None:
    agent_identity.establish(42, owns_loop=True)
...
def test_try_establish_from_env_sets_identity(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert agent_identity._owns_loop is False  # env-established scripts don't own the loop
...
def test_try_establish_from_env_noop_when_already_set(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert agent_identity._owns_loop is True  # unchanged
...
def test_require_actor_derives_agent_when_no_actor() -> None:
    agent_identity.establish(42, owns_loop=True)  # no establish_actor
...
def test_actor_takes_precedence_over_agent_id() -> None:
    ...
    agent_identity.establish_actor("schedule:9")
...
def test_require_actor_establishes_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AVA_AGENT_ID", "42")
    assert agent_identity.require_actor() == "agent:42"
    assert agent_identity._owns_loop is False  # env-established, not loop owner
