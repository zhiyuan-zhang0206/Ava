...
def _insert_page_row(
    ...
    port: int,
    *,
    serve_dir: str | None = "/tmp/serve",  # noqa: S108
    host: str = _HOST,
) -> None:
...
class TestOpenRows:
    def test_filters_by_host_and_serve_dir(self, sync_pool, db_conn: psycopg.Connection) -> None:  # pyright: ignore[reportMissingParameterType, reportUnknownParameterType]
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "managed", 12001)
        _insert_page_row(db_conn, agent_id, "other-host", 12002, host="10.0.0.9")
        _insert_page_row(db_conn, agent_id, "agent-owned", 12003, serve_dir=None)
        _insert_page_row(db_conn, agent_id, "closed", 12004)
...
class TestReconcile:
    def test_spawns_missing_and_kills_closed(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "a", 12011)
        _insert_page_row(db_conn, agent_id, "b", 12012)

        managed: dict[tuple[int, str], psd._ServerHandle] = {}
        backoff: dict[tuple[int, str], float] = {}
        psd._reconcile_once(sync_pool, managed, backoff, _HOST, Path("/tmp/logs"))  # noqa: S108  # pyright: ignore[reportUnknownArgumentType]
    ...
    def test_respawns_dead_process(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12021)
    ...
    def test_backoff_after_failed_spawn(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12031)
...
class TestReclaim:
    ...
    def test_reclaims_wanted_port_occupant(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12041)
    ...
    def test_kills_orphan_of_closed_row(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "gone", 12042)
    ...
    def test_never_kills_own_managed_process(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12043)
        own = _stub_handle(12043, agent_id=agent_id)
    ...
    def test_skips_foreign_cluster_occupant(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12044)
        killed: list[int] = []
        managed: dict[tuple[int, str], psd._ServerHandle] = {}
        backoff: dict[tuple[int, str], float] = {}
    ...
    def test_skips_occupant_with_unreadable_env(
        ...
        agent_id = spawn_agent()
        _insert_page_row(db_conn, agent_id, "p", 12045)
        killed: list[int] = []
        managed: dict[tuple[int, str], psd._ServerHandle] = {}
        backoff: dict[tuple[int, str], float] = {}
