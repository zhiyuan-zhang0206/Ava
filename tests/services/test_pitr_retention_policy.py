    ...
    for name in required_archive_names(candidate.wal_ranges, SEGMENT):
        if name.endswith(".history"):
            item = RetentionObject(
                f"pitr/wal/{name[:8]}/{name}.enc", 100 + candidate.timeline, 20, name, "history"
            )
        else:
        ...
        wal.append(
            RestoreObject(
                item.archive_name or "", item.object_name, item.generation, item.size, "crc", ()
            )
        )
...
def test_gap_or_generation_replacement_blocks_every_eligible_object() -> None:
    ...
    plan = plan_retention(
        RetentionEvidence(
            (first, second, third), tuple(map(_proof, (first, second, third))), inventory
        )
    )
...
def test_later_timeline_history_is_always_ancestry_pinned() -> None:
    ...
    history = RetentionObject(
        "pitr/wal/00000002/00000002.history.enc", 30, 20, "00000002.history", "history"
    )
    ...
    plan = plan_retention(RetentionEvidence((first, second), proofs, inventory))
    assert any(
