...
def test_missing_approved_pin_is_reported(monkeypatch):
    query = Mock(
        side_effect=[
...
def test_command_failure_is_not_an_empty_pin_set(monkeypatch):
...
def test_pin_observation_uses_real_bounded_process_contract(tmp_path, monkeypatch):
...
def test_nonzero_brew_query_is_not_readiness(tmp_path, monkeypatch):
    executable = tmp_path / "brew"
    executable.write_text("#!/bin/sh\nexit 7\n")
    executable.chmod(0o700)
    monkeypatch.setenv("PATH", str(tmp_path))
