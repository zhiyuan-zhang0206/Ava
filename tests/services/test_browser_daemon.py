...
def test_chrome_args_opens_frontend_as_first_tab_when_configured(tmp_path: Path) -> None:
    ...
    --user-data-dir)."""
    args = bd._chrome_args("/chrome", 9222, tmp_path / "prof", "http://localhost:3001")
    assert args[-1] == "http://localhost:3001"
    assert "--remote-debugging-port=9222" in args
    assert f"--user-data-dir={tmp_path / 'prof'}" in args
...
def test_frontend_url_reads_app_port_from_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    """The first-tab URL is the configured app port — never hardcoded; an
    agent-runner-only host (app_port unset) gets no first tab."""
    monkeypatch.setattr(settings.services, "app_port", 3001)
    assert bd._frontend_url() == "http://localhost:3001"
