...
class _ChunkedStream(httpx.AsyncByteStream):
    """Deliver the body in caller-controlled chunks."""
...
@pytest.mark.parametrize("ch", ("\u0085", "\u2028", "\u2029"), ids=("U+0085", "U+2028", "U+2029"))
def test_stream_events_decodes_char_split_across_chunks(ch: str) -> None:
    """A multi-byte character split across transport chunks must decode intact.

    A per-chunk decode() replaces its half-sequences with U+FFFD and the
    payload is corrupted while still parsing as JSON.
    """
    payload = {"role": "timeline_snapshot", "content": f"a{ch}b"}
    encoded = ("data: " + json.dumps(payload, ensure_ascii=False) + "\n\n").encode()
    cut = encoded.index(ch.encode()) + 1  # split inside the character
