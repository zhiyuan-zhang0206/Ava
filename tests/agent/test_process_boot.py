...
def test_eval_isolation_applied_after_plugin_registration_in_build_graph() -> None:
    code, out, err = _run("""
        import asyncio
        import os
        ...
        import agent._process_boot as process_boot

        def fake_build_graph(_checkpointer):
            ava.register_namespace(
                "memory",
                SimpleNamespace(
                    PATH=ava.const(Path("/shared-memory"), doc="Shared memory path."),
                    search=lambda _query: ["shared result"],
                    search_detailed=lambda _query: ["shared result"],
                    __doc__="Shared memory namespace.",
                ),
            )
            ava.register_namespace("tasks", SimpleNamespace(list=lambda: []))
            return SimpleNamespace()

        async def no_repair(_graph, _agent_id):
            return None

        process_boot.build_graph = fake_build_graph
        process_boot._repair_dangling_tool_use_at_startup = no_repair
        process_boot._write_effective_config_to_restart_completed = lambda _agent_id: None

        asyncio.run(process_boot._build_graph(None, None, 417))

        assert ava.memory.PATH.name == "memory-pool"
        assert ava.memory.search("x") == []
        assert not hasattr(ava, "tasks")
        assert not hasattr(ava, "web")
        assert not hasattr(ava, "understand")
        assert not hasattr(ava.agents, "get_last_message")
