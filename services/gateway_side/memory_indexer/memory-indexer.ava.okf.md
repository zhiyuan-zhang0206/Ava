...
- **Batch liveness**: `_process_paths` beats at entry and before every embed, delete, stale-row cleanup and upsert, on both success and partial failure. The ceiling is `max(180s, provider worst batch + 30s)`. The factory pairs each provider with its budget. Each Gemini attempt, sync or async, runs under an asyncio cancellation deadline equal to the configured timeout; the budget includes retry backoff, capped Retry-After and both jitter terms. Defaults: 606s batch / 636s ceiling. Beats prevent multiple embeds, commit tails or delete-only batches accumulating past the ceiling; NumPy's 300s upsert fits independently. The margin covers scheduling, local processing and loop resumption; the floor preserves other branches' slack. True wedges take longer to detect: false kills cost a rebuild, delayed detection costs staleness while search reads the existing index.
...
- Schema mismatch (legacy path-PK collection, missing `embedder` column, or a vector width other than the configured provider's dim) is detected at connect: writable Milvus daemon connections drop + recreate the collection, then cold-start rebuilds the index chunked; read-only reconcile-tool connections refuse a missing or mismatched collection without persistent mutation
