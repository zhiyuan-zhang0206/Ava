...
- **Batch liveness**: `_process_paths` beats at entry and immediately before each `embed_batch`, for both reconcile and drain work. The ceiling is `max(180s, provider worst batch + 30s)`: the provider factory registers each constructor with its batch budget; Gemini derives that budget from configured request timeout, retry attempts, backoff, the shared Retry-After cap, and both jitter terms (606s batch / 636s ceiling with defaults). This covers a single call exceeding the historic 180s ceiling and several shorter calls accumulating within one `_process_paths`. The 30s margin covers executor scheduling, the successful/failed post-batch file commit, and loop resumption; the 180s floor preserves slack for other loop branches. Detection of a true wedge is later: a false kill costs a rebuild, while delayed detection costs staleness and search keeps reading the existing index
