...
- **Two retry policies, split by call site** (`embedder.py`): `_EMBED_POLICY` (4 attempts, 1→2→4→8s) for the daemon's document embeds — no caller deadline, resilience = the retry; `_QUERY_EMBED_POLICY` (2 attempts, 1s) for search query embeds, which run inside the gateway endpoint's own deadline and are retried by the caller, not the embed
