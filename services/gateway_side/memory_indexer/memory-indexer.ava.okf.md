...
- **Batch liveness**: `_process_paths` beats at entry and before every embed, delete, stale-row cleanup and upsert, on both success and partial failure. The ceiling is `max(180s, provider worst batch + 30s)`. The factory pairs each provider with its budget: Gemini budgets twice the configured timeout per sync attempt (raw body deadline plus one read window), retry backoff, capped Retry-After and both jitter terms; async attempts use cancellation deadlines. Defaults: 846s batch / 876s ceiling. Beats prevent multiple embeds, commit tails or delete-only batches accumulating past the ceiling; NumPy's 300s upsert fits independently. The margin covers scheduling, local processing and loop resumption; the floor preserves other branches' slack. True wedges take longer to detect: false kills cost a rebuild, delayed detection costs staleness while search reads the existing index. Boundary (2026-09-20 ruling): sync deadlines cover body streaming; headers keep HTTPX per-op timeouts (drip-header overrun possible; transport-level fix out of scope).
