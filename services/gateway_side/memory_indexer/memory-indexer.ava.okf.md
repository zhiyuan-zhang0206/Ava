...
- Schema mismatch (legacy path-PK collection, missing `embedder` column, or a vector width other than the configured provider's dim) is detected at connect and the collection is dropped + recreated; cold-start then rebuilds the index chunked
