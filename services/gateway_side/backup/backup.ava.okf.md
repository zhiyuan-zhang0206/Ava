...
- **Atomic write**: writes private plaintext and encrypted `.partial` files first, then publishes the encrypted artifact only after both commands succeed — half-finished dumps won't be mistaken for complete backups by due/prune logic or manual recovery. A failed or interrupted run keeps its partials as evidence; no later run sweeps them.
- **Owned operation**: the scheduled dump runs in one directly owned worker group (`services/backup_scheduler/worker.py`, see [[shutdown.ava.okf.md|Shutdown ownership]]). The worker writes and best-effort publishes off-site inside `$AVA_HOME/backups/operations/`; the controller links the exact artifact into the backup directory and prunes only after the group closed and the worker exited 0 with a matching SHA-256. A failed or cancelled run leaves its controls, and the next run refuses until an operator retires them.
...
- `services/backup.py` — `prepare_scheduled_backup()` / `commit_scheduled_backup()` for the scheduler's operation; `run_backup()` dumps, publishes and prunes in-process for snapshot callers
