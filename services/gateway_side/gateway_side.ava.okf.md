...
- **frontend** — Next.js frontend (`frontend/` subtree)
- **gateway-watchdog** — the watchdog instance for the gateway capability (`--role gateway`; see [[watchdog.ava.okf.md]], one per side)
- **gate** — the always-up fleet UI entry (`services/gate/daemon.py`). Deliberately **not a `ServiceSpec`** and therefore not in the table above: it is a launchd KeepAlive job (a detached pidfile-backed process off macOS) registered by `cli/commands/_converge_gate.py`, sitting OUTSIDE the update lifecycle so the entry port never blacks out during a rollout — it owns :3000 and proxies the Next.js app on :3001, serving the login page when unauthenticated and — when the gateway is unreachable — one of two maintenance pages picked by the posture mirror (`$AVA_HOME/deploy-state.json`, written by the same transitions that write the posture row, R1 Task #1021): the updating page while a rollout is in flight, and a distinct "service unavailable" page when the mirror is idle/absent, i.e. the machine itself is down. Those three pages (`services/gate/static/`) are dependency-free rebuilds of the app's own `/login` and `UpdatingPage` screens — the app's design tokens are **copied** into `theme.css` and the user's next-themes choice is read off the shared origin's `theme` localStorage key, so the swap a rollout performs is invisible; the copy is the price of a page that must render with the app dead, and it has to be moved by hand when the app's palette moves — but only the *copying* is manual: the plist carries a content hash of `services/gate/`, so once the tokens are synced the next converge replaces the launchd job and the new pages are live, no hand restart. Being outside the roster means no session row and no watchdog keepalive covers it, so its own two observations (entry answering / supervisor still holding it) are shown by `ava status` and alerted by health-probe check 5
