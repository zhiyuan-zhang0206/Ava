...
- **Reject a non-label**: `labeler.py:_rejection_reason()` classifies an output that is not a label at all (opens with a tag; opens in the assistant's first-person voice) as a FAILED generation — `generate_label_async` returns `False` and the daemon's existing exponential backoff retries. Rejection, not repair: `_normalize` still refuses to rewrite a bad output
