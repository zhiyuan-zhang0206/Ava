...
- **Loki→PG day-grain rollup** (`services/events_maintenance/rollup.py:compute_rollup`, unconditional): aggregates whole retained days up to yesterday (UTC) from Loki (`_day_aggregates` is the single Loki-facing seam) into `agent_metrics_daily` (per agent × UTC-day turn/exec counts + mergeable turn-duration sum/min/max; p50/p90 deliberately excluded — not mergeable across days) and `agent_model_tokens_daily` (per agent × UTC-day × model token counts + `cost_usd` = sum of usage-time price snapshots, `costed_calls` / `unpriced_calls` — **no read-time pricing**; a snapshot-less call is counted unpriced at zero cost). Full-day overwrite upsert keyed on the PK ⇒ idempotent; restarts or fast intervals never double-count. A zero-row indexed slice is not authoritative evidence that the day is empty: that day is warned and skipped, preserving existing ledger rows while the pass continues.
