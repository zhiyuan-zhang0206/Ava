...
core's per-chat ``ChatState`` (duck-typed via the core module, which is only
touched at runtime — the import cycle is resolved by lazy attribute access).
...
from services.im_bridge import core as _core
...
class SpawnMenuMixin:
    ...
    gateway: _core.GatewayClient

    async def _cmd_spawn(self, state: _core.ChatState) -> _core.Reply:
    ...
    async def _spawn_layer_preset(self, state: _core.ChatState) -> _core.Reply:
        ...
        return _core.Reply(copy.SPAWN_LAYER_PRESET, buttons=[*buttons, _spawn_button(draft)])

    async def _spawn_layer_model(self, state: _core.ChatState) -> _core.Reply:
        draft = state.spawn_draft or SpawnDraft()
        state.spawn_draft = draft
        data = await self.gateway.list_models()
        models = sorted(data.get("models", {}).keys())
        if not models:
            return _core.Reply(copy.SPAWN_NO_MODELS)
        buttons = [(model, f"spawn:model:{model}") for model in models]
        return _core.Reply(copy.SPAWN_LAYER_MODEL, buttons=[*buttons, _spawn_button(draft)])

    async def _spawn_layer_effort(self, state: _core.ChatState) -> _core.Reply:
        draft = state.spawn_draft or SpawnDraft()
        state.spawn_draft = draft
        ...
        return _core.Reply(copy.SPAWN_LAYER_EFFORT, buttons=[*buttons, _spawn_button(draft)])
    ...
    async def _handle_spawn_menu(
        self, state: _core.ChatState, text: str
    ) -> _core.Reply | list[_core.Reply]:
        ...
        if action == "preset":
            if value == "none":
                ...
                if picked is None:
                    return _core.Reply(copy.SPAWN_PRESET_GONE)
        ...
        return _core.Reply(copy.SPAWN_UNKNOWN_ACTION)

    async def _spawn_execute(self, state: _core.ChatState) -> _core.Reply:
        draft = state.spawn_draft
        state.spawn_draft = None
        if draft is None:
            return _core.Reply(copy.SPAWN_NOTHING_TO_SPAWN)
        ...
        try:
            ...
            return _core.Reply(copy.SPAWN_FAILED.format(exc=exc))
