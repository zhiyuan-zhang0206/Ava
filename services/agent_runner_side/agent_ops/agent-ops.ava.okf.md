...
- **Inbound HTTP endpoint**: binds `0.0.0.0:<ops_port>`, serves `POST /ops`; `GET /healthz` (watchdog health check) on the same port, via localhost without auth.
- **Always authenticated**: every `/ops` must carry the cluster secret (`AVA_CLUSTER_SECRET`) as bearer — even single-machine gateway dialing its own /ops uses the same auth (reachability ≠ trust); daemon refuses to start if secret is missing.
...
- **work kinds**: `spawn` / `lifecycle` / `cluster_stop` / `cluster_update` / `cluster_resume` / `status_probe` / `config_read|write` / `inventory_read|write`.
