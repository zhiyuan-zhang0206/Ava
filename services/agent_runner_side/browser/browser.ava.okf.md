...
- **Shared MCP upstream**: `mcp_daemon.py` shares one `chrome-devtools-mcp` across agents. It also logs in to the gateway, injects the opaque server session into Chrome over CDP, and refreshes it every six hours.
