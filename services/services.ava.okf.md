...
- The service keep-alive roster **single source of truth** = `build_services()`'s `ServiceSpec.healthcheck_module`: the watchdog derives from it (`_checks_for_capability`), no longer a second hardcoded list; registering a new service in `build_services()` automatically gets 60s keep-alive. The two data-plane pseudo-checks (`redis-acl` / `pgbouncer`) are manually attached; host checks (`brew-pin` / `lgtm`) are also manual. pg-backup is a regular supervised service.
...
- **permissions-helper identity and updates**: `build-state.json` binds the installed app to its source-content hash and exact designated requirement; converge refuses a signed bundle whose requirement drifts because macOS may treat it as a new TCC identity. A rebuilt running helper receives `self_upgrade` and execs the installed executable in place, preserving its PID, grants, and nursery children; older or failed helpers fall back to launchd kickstart. Every converge pings the loaded helper and repairs one `LWCR` / `EX_CONFIG` spawn-failed state with bootout + bootstrap before failing. The first converge from a checkout-era install moves a valid signed bundle into `$AVA_HOME/helper` and rewrites the LaunchAgent path without rebuilding it.
