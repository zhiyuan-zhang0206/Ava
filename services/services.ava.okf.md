...
- **permissions-helper identity and updates**: `build-state.json` binds the app to its source hash and designated requirement; converge refuses requirement drift because macOS may treat it as a new TCC identity. A rebuilt helper receives `self_upgrade`, preserving its PID, grants, and children; older helpers fall back to launchd kickstart. Converge and the debounced watchdog call `repair_unresponsive_helper()` for one bootout/bootstrap repair. A valid checkout-era bundle moves into `$AVA_HOME/helper` without rebuilding.
