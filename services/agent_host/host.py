    ...
    async def _run_turn(self, agent_id: int) -> None:
        ...
        async with self._turn_slots:
            ...
            if incarnation is None:
                ...
                return
            exited = False
            # Admission is durable before its optional live announce. Put every
            # await after that commit inside the same settlement boundary: a
            # cancelled or half-open Redis publish must not strand a false
            # `running` row that the owner's lease then keeps alive.
            self._in_flight.add(agent_id)
            try:
