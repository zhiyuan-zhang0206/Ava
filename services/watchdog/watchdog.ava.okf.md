...
2. **paused posture** (`host_deploy_state.posture` = paused/converging) — written at service shutdown (and by the updater's lease claim), skip if hit so healthchecks cannot revive stopped services. The native maintenance journal also gates healthchecks throughout drain; it has no expiry and stranded-pause recovery cannot release it. **Stranded-pause recovery**: a pause with no owner — no *executing* update lease AND no live local updater lease — for over `STRANDED_PAUSE_TIMEOUT_S` (2m) → judges the orchestration hard-killed and unpauses itself only when the exact updater recovery envelope is terminal-clearable. The age is measured from `paused_at`, not `updated_at` (lease renewals bump the latter every round — the 122-round 2026-09-10 blindness, issue #2101). Planned, unfinished, or malformed bootstrap/normal evidence remains paused for checked recovery. An OWNED pause is declined however long it runs; its owner states end on their own (the lease TTL, or the stalled-updater reaper killing a silent session). Reading only the cluster lease missed a watchdog-spawned updater entirely, which is why the bound had to be 10min and why a failed updater's pause outlived it (issue #1074). The lease is asked `DeployLease.awaits(machine_name())`, not "is it held": a **settle hold naming this host** is not ownership — nothing runs under one, and it exists only because the orchestration proved this pause lost its owner. Deferring deadlocked the very host it waited for, since this gate blocks `ALL` and the gates that converge it never ran (issue #1116). Lifecycle-mutex contention during recovery (`LockTimeoutError` on `deploy-state.lifecycle.lock`) is expected, not fatal (issue #1943): the round cannot prove the pause is unowned, so recovery defers with the pause kept — the watchdog stays alive and healthy; only real I/O errors raise. The 2026-09-10 additions — the `paused_at` age anchor, gateway-reachability evidence against a live lease, the pause-scoped exemption of the gateway healthcheck (respawn gated on the owner determination), and the off-pin converge-back — live in [[pause-recovery/pause-recovery.ava.okf.md]].
...
4. **pin drift** — HEAD ≠ cluster pin (`cluster_target_sha`). Capability division: **agent-runner** watchdog acts (force-update to pin, with persistent backoff to prevent infinite loop on bad pin); **gateway** watchdog only alerts (gateway drift requires full rollout, not single-node self-heal) — except the gateway healthcheck's respawn, which converges a drifted checkout back to the pin first when the gateway is down (guarded, pin never written; [[pause-recovery/pause-recovery.ava.okf.md]]).
