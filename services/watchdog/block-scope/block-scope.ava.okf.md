...
The two hand-added pseudo-checks classify themselves the same way: `redis-acl` is redis-only (`False`, so an ACL repair survives a pg outage), `pg-backup` dumps the database (`True`).
...
The restarter's own controllers (respawn / hibernate / resurrect / wedged) are never gates and always report `NONE`.
...
`browser`, `browser-mcp`, `milvus`, `memory-indexer`, `frontend`, plus the `redis-acl` pseudo-check. Everything else on the roster calls `assert_schema_current` at boot and then reads or writes the DB.
