...
from services.pitr.worker_process import publish_result, worker_request
...
def run(raw: dict[str, Any], output_path: Path) -> None:
    expected = {
        "candidate_json",
        "root",
        "ack_dir",
    ...
    if "drill" in raw:
        _run_drill(raw, candidate, group.generation_pinned_object_reader(), output_path)
        return
