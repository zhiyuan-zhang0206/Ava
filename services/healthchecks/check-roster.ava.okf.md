...
| `pitr_uploader.py` | PITR GCS uploader | HTTP `/healthz` (identity-verified), single-worker loop heartbeat | `respawn_and_verify` | uploader event loop remains responsive while local WAL waits for verified remote ACK |
...
| `otel_collector.py` | OTel collector sidecar | POST `/v1/traces` to the OTLP receiver must return 2xx, and listeners on :4318/:8888 must resolve to this unit's collector binary + live session record; an unrecorded same-binary holder is stale/down, while another holder is terminal; non-LGTM gateways warn and return before probing/restarting, while pure runners retain relay behavior | give each verified stale holder one 5 s SIGTERM window before a verified SIGKILL fallback, then `respawn_and_verify` through the same 5 s session-stop ladder; a survivor remains loud/down | the supervisor-owned OTLP listener the agents export through answers, rather than an old collector that happens to retain the port; non-LGTM gateway homes do not claim a collector they cannot export from |
