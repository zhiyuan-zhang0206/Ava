...
| `otel_collector.py` | OTel collector sidecar | POST `/v1/traces` to the OTLP receiver must return 2xx, and the listeners on :4318/:8888 must resolve to this unit's collector binary + live session record; an unrecorded same-binary holder is stale/down, while another holder is terminal | reclaim the verified stale holder, then `respawn_and_verify` | the supervisor-owned OTLP listener the agents export through answers, rather than an old collector that happens to retain the port |
