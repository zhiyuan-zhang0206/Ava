...
| `lgtm.py` | LGTM backend (docker compose, `deploy/lgtm/`) | four readiness endpoints (Loki/Prometheus/Tempo/Grafana) on the compose's fixed host ports; any HTTP answer = alive, connection failure = down; no-op without the `$AVA_HOME/lgtm-host` marker | re-run the idempotent `deploy/lgtm/start.sh` | each container's readiness listener answers (its own health traversal) |
