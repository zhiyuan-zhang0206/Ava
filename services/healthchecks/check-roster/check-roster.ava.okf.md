...
| `frontend.py` | Next.js | HTTP 2xx on the **app** port (not the entry port — the entry's always-up gate answers 200 while the app is down), **and the port's LISTEN socket must resolve to the frontend's owner** (session or tree unit; leader or birth-validated descendant); anyone else's 2xx reads `PORT_TAKEN`, naming the pid | kill session + `npm run build && npm run start`; refused while a listener outside the session holds the app port (no EADDRINUSE respawn loop) | the Next.js app **of this unit** renders — an orphan's 200 cannot mask a failed start |
