...
| app port answered, LISTEN socket outside the frontend's expected owner (session or tree unit) | `PORT_TAKEN` | an orphan of a dead session (or a foreign process) holds the port — `kill_session` frees nothing, so the watchdog refuses to respawn into it; recovery is converging the recorded group (`pause`/`stop`) |
...
The frontend row resolves ownership the same way — its 2xx proves a renderer, not ownership: the app-port LISTEN socket must resolve to a pid of the frontend's expected owner — the current session, or the ava-root tree unit on a root-driven host (`services/healthchecks/frontend.py`, task #3370).
