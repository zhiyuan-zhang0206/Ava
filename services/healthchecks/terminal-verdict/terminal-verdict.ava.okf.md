...
`shared.service_respawn.run_keepalive` is the one place the policy lives — the shared body of every daemon healthcheck's `main()`. On a terminal verdict it logs at **ERROR** naming the occupying home, then exits `EXIT_PORT_TAKEN` (3), distinct from `EXIT_RESPAWN_FAILED` (1, "respawned and it did not come up — retried next round"). The watchdog's `_run_check` catches `SystemExit` and logs the code, so the distinction lands in that capability's watchdog log.

It is **loud-and-stop, never silent-no-op**: a healthcheck that quietly declines to heal is the shape of the 98-minute outage ([[services/healthchecks/probe-contract/probe-contract.ava.okf.md]]). Remediation that does not depend on the respawn still runs — the restarter's stand-in dispatch is passed as `on_unrevivable` and fires on the terminal path too, because with the daemon unrevivable it is the only thing still moving `restarting` rows. That hook is called only after a respawn attempt, or in place of one that would be futile, which is where the restarter's "nothing failable ahead of the respawn" ordering invariant now lives.
