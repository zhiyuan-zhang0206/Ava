...
- **Eight hand-added pseudo-checks have no ServiceSpec**: `brew-pin` covers host policy; `prod-venv` reports production dependency metadata and isolated import failures on POSIX without repair; agent-runner `permissions-helper` covers the launchd app; agent-runner `browser-reach` covers the shared Chrome's own network face to the gateway (report-only); `pgbouncer` / `redis-acl` cover native data-plane processes; `lgtm` covers native backends through readiness plus a Loki write/read probe; gateway `station-probe` covers the remote observatory. Only station-probe requires DB (to resolve `machine_units`). PgBouncer probe scope: [[services/healthchecks/check-roster/roster-notes.ava.okf.md]].
- **permissions-helper exception**: its watchdog keeps episode state in memory, pings the real protocol (3 s), and on failure classifies the launchd `job state` — LWCR-class = `spawn failed` plus a 78 exit or `needs LWCR update`; the top-level `state` never carries it (F5 findings section 6). A failed repair escalates (`permissions_helper_repair_failed`) and retries under backoff (5 min → 1 h) instead of sealing; one `permissions_helper_unhealthy` alert per episode, reset only by a healthy round. The module's total `probe()` is the root era's entry (task #3393).
