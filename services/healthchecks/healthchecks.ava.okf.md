...
- **Restart trigger**: `run_keepalive` rebuilds a session after a respawnable failure; terminal verdicts report
...
- **The hand-added checks (`brew-pin` / `pgbouncer` / `redis-acl` / `lgtm`) have no ServiceSpec**: brew-pin asserts host package policy on both capabilities, pgbouncer and redis are native per-cluster processes, and the LGTM backends are native host processes — none are sessions, so the watchdog adds them by hand (redis-acl + pgbouncer prepended, brew-pin on both roles, lgtm appended). All four carry `requires_db=False`, since a DB-scoped block exists exactly when the database is unreachable and holding back these repairs would deadlock (data plane), blind the operator (observability), or suppress host-local pin detection. The pgbouncer check probes twice — the loopback admin console plus a local socket-table check for the configured reachable address (`pgbouncer_public_listener_reachable`, task #1288): pgbouncer keeps running when one `listen_addr` entry fails to bind, so a loopback-only protocol probe reads a silently degraded pooler as healthy, while the socket table detects the missing public bind without a network hairpin self-dial; that missing listener is a repair condition (restart) just like a dead pooler
...
- **Traversal audit (issue #192, 2026-08-21)**: every check must traverse what it certifies — a check that shares no code path with what breaks stays green through the outage (the 0004 lesson; `conventions/defensive-patterns.md`). The audit found the rule respected by 20 of the then-21 checks; `milvus.py` was the one port-open-only probe (a bare TCP connect) and was upgraded to a real `list_collections` RPC. The later `brew_pin.py` assertion reads Homebrew's pin roster directly. The roster table carries a "what it certifies" column, and `scripts/lint_doc_roster.py` pins the table to the module directory + ServiceSpec + hand-added registrations (set equality) so the drift found in the audit cannot silently return.
