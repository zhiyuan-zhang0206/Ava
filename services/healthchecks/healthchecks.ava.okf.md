...
- **The two data-plane checks (`pgbouncer` / `redis-acl`) have no ServiceSpec**: pgbouncer and redis are native per-cluster processes, not sessions, so the watchdog prepends both by hand ahead of the derived roster. pgbouncer runs FIRST among the services because when it is enabled it IS every consumer's `AVA_DB_URL` — reviving a daemon ahead of it only produces one that cannot reach the database — and both carry `requires_db=False`, since a DB-scoped block exists exactly when the database is unreachable and holding back the repair of its front door would deadlock
