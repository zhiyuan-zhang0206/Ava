...
- **Independent liveness logic**: each service defines its own "alive" criteria (HTTP `/healthz` or `/api/health` 200 / CDP / unix socket ping / real RPC via `MilvusClient` / redis PING) — there are no longer any pidfile-probed or port-open-only services in the table
...
- **The hand-added checks (`pgbouncer` / `redis-acl` / `lgtm`) have no ServiceSpec**: pgbouncer and redis are native per-cluster processes, and the LGTM backends are native host processes — none are sessions — so the watchdog adds them by hand (redis-acl + pgbouncer prepended, lgtm appended). pgbouncer runs FIRST because when enabled it IS every consumer's `AVA_DB_URL`; all three carry `requires_db=False`, since a DB-scoped block exists exactly when the database is unreachable and holding back these repairs would deadlock (data plane) or blind the operator (observability). The pgbouncer check probes twice — the loopback admin console and the configured reachable address (`pgbouncer_public_listener_reachable`, task #1288): pgbouncer keeps running when one `listen_addr` entry fails to bind, so a loopback-only probe reads a silently degraded pooler as healthy, and the missing public listener is a repair condition (restart) just like a dead pooler
...
- **Not every dead verdict is respawnable** — a `/healthz` answered by another unit's daemon is terminal: reported at ERROR with a distinct exit code and NOT respawned, because no respawn this unit can perform frees that port. The three-way verdict and where the line falls: [[services/healthchecks/terminal-verdict/terminal-verdict.ava.okf.md]]. The shared `main()` that applies it is `shared.service_respawn.run_keepalive`, used by all eleven keepalive healthchecks (gateway, ops, restarter, labeler, heartbeat, memory-indexer, events-maintenance, delivery-watchdog, im-bridge, agent-host, page-server).
...
- **Traversal audit (issue #192, 2026-08-21)**: every check must traverse what it certifies — a check that shares no code path with what breaks stays green through the outage (the 0004 lesson; `conventions/defensive-patterns.md`). The audit found the rule respected by 20 of 21 checks; `milvus.py` was the one port-open-only probe (a bare TCP connect) and was upgraded to a real `list_collections` RPC. The roster table now carries a "what it certifies" column, and `scripts/lint_doc_roster.py` pins the table to the module directory + ServiceSpec + hand-added registrations (set equality) so the drift found in the audit cannot silently return.
