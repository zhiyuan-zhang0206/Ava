...
def _peer_pid(sock: socket.socket) -> int:
    """Ask the connected Unix stream's kernel peer, not a path or ping claim."""
    if sys.platform == "darwin":
        # Darwin SDK sys/un.h: SOL_LOCAL=0, LOCAL_PEERPID=2.
        return sock.getsockopt(0, 2)
    if sys.platform == "linux":
        return struct.unpack("3i", sock.getsockopt(socket.SOL_SOCKET, socket.SO_PEERCRED, 12))[0]
    raise RuntimeError("native Unix peer PID inspection is unavailable on this platform")


def _owned_ping(resolve_owner: Callable[[], OwnedProcess | None], path: Path) -> DaemonProbe:
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as sock:
        ...
        peer = members.get(_peer_pid(sock))
