...
-- Lineage ties (spawn/fork/resurrect) are permanent; message ties decay.
-- Reads category=audit rows of the unified events table (the successor of
-- event_log). Rebuilt for the unified stream by
-- migrations/20260804T203036_events-readers-neighbors.sql.
...
LANGUAGE sql STABLE AS $$
    WITH RECURSIVE
    -- One undirected edge per unordered pair {a,b}: permanent lineage weight
    -- (LN(1+count), no decay) summed with the decayed message weight. Reads
    -- the unified events stream (category=audit), the successor of event_log.
    edge AS (
