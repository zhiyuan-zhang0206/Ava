...
-- A live tree node always points at its nearest living ancestor. When an
-- intermediate agent terminates, preserve its terminated descendants' visible
-- lineage but fold each live child over that now-dead node. Fork provenance is
-- deliberately separate (fork_source_agent_id) and remains untouched.
...
-- Terminated descendants deliberately retain their historical parent for the
-- show-terminated tree. If one returns to a live status, repair its tree
-- parent before it can reappear beneath that terminated ancestor.
