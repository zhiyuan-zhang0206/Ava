...
- `pause_heartbeat(duration: float)` — Pause idle heartbeat for `duration` seconds (0, 86400]. For waiting on a known external event only (watcher / peer reply / scheduled callback / user answer), never as a substitute for terminating when the work is done; successive pause windows for the same wait must strictly increase (30m → 2h → 4h → 8h → 16h → 24h cap) — repeating or shortening the window is a backoff violation, and the call buffers an in-exec reminder note when it does (user ruling 2026-08-29). Only suppresses heartbeats, real wake-ups still arrive; subsequent calls replace the window.
