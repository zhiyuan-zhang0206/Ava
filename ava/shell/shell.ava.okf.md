...
- `run_background(cmd, *, name, cwd=None, keep=False, ttl, notify="always") → BackgroundRun` — Run a long command in a new persistent session, return immediately, then the session auto-closes. `notify="always"` (default) sends a completion message for every exit; `notify="failure"` sends it only for non-zero exits. The message format is exit code + log path + output tail. Output is streamed to `.shell_logs/<sid>_<name>.log` in workspace (relative path, can read/grep to view progress) and visible live in the session capture. `keep=True` retains the session after command ends. `ttl` max 86400s (24 hours). Use for one-shot long tasks like build/test/download; interactive programs still use `sessions`. Returns `BackgroundRun(session_id, output_path)`.
