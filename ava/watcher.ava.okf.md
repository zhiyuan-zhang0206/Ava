...
- `cron(expr, message, *, timezone=None, end_time=None, name) → int` — Periodic wake-up via 5-field cron expression. `end_time` sets expiry; it **defaults to now + 7 days** (standing cap, task #2617) — a longer schedule passes an explicit `end_time`. Re-registering the same expression + timezone **supersedes**: the old watcher is replaced (new session, fresh end), never stacked — **regardless of either end time** (task #2617 renewal and #2061 explicit-end supersede, user ruling 2026-09-10: one live watcher per schedule). Returns session ID.
