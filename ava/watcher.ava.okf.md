...
- `cron(expr, message, *, timezone=None, end_time=None, name) → int` — Periodic wake-up via 5-field cron expression. `end_time` sets expiry; it **defaults to now + 7 days** (standing cap, task #2617) — a longer schedule passes an explicit `end_time`. Re-registering the same expression + timezone **renews**: the old watcher is superseded (new session, fresh window), never stacked. Renewal supersedes a same-schedule watcher **regardless of its end time** — a defaulted re-registration replaces even an explicit longer-end watcher; the longer intent must re-register explicitly. Returns session ID.
