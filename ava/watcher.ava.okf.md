...
- `at(when, message, *, name, notify="always") → int` — One-time scheduled wake-up. `when` supports TZ-aware datetime / timedelta / ISO-8601 strings. Returns session ID (can be cancelled with kill).
- `cron(expr, message, *, timezone=None, end_time=None, name, notify="always") → int` — Periodic wake-up via 5-field cron expression. `end_time` sets expiry; it **defaults to now + 7 days** (standing cap, task #2617) — a longer schedule passes an explicit `end_time`. Re-registering the same expression + timezone **supersedes**: the old watcher is replaced (new session, fresh end), never stacked — **regardless of either end time** (task #2617 renewal and #2061 explicit-end supersede, user ruling 2026-09-10: one live watcher per schedule). Returns session ID.
- `launch(code, timeout, *, name, notify="always") → int` — Run custom Python code as watcher. Inside the code, call `ava.agents.send_message(ava.self.AGENT_ID, content)` to wake you. Force-stop after `timeout` (exit code 124).

All watcher APIs accept `notify="always"` (default) to send their completion message for every exit, or `notify="failure"` to send it only for a non-zero exit. The shell-level message format and session lifecycle do not change.
...
A rebuilt watcher (boot-reconcile revival after a reap, or a template upgrade) re-registers without `notify`: the completion policy falls back to `"always"` until it is re-registered — the policy is not persisted yet (task #4410).

A dead cron row is rebuilt only when NO live same-schedule row exists — the reconcile's dedupe is **schedule-level** (expression + timezone, any end time): a live row subsumes a dead twin's rebuild, so a supersede whose kill half failed cannot stack a second watcher (QA review of PR #2037). Registration is the mirror image: the supersede pass runs first for EVERY registration (any end), then the exact-end dedupe — an identical live schedule is reused, a different-end live twin is superseded (#2061), never stacked.

A watcher whose owner agent is **terminated for good** (never auto-resurrect-eligible) is reclaimed by the gateway ttl-reaper instead: the session is killed via the `shell_kill` op — or, when the owner's machine no longer has a registry row, terminalized directly without a kill (mode `machine_absent`, task #4143) — and the row marked `reaped` — with the owner re-checked in the same statement, so a mid-flight resurrect leaves the row `running` for the agent's own reconcile (task #2617). Each definitive reap queues a reclamation notice for the owner in the same transaction: `reaped` is terminal and never rebuilt, so the notice (delivered on the agent's next resurrect, never resurrecting it itself) is what tells it to re-register the schedule if still needed (#2060, user ruling 2026-09-10). Crash corpses (`reaper` / `launch-confirm` termination sources) keep their watchers — they are auto-resurrect-eligible and their own wakes are a revival channel.
