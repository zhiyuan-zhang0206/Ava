...
## Key Dependencies
- [[system-prompt.ava.okf.md]] — the skill index is injected into the system prompt by the **agent process itself** (`agent/graph/_capabilities.py:_skill_index_lines` scans local `~/.ava/skills`; `_system_prompt.py` imports the rendered sections from `_capabilities`), once, into `# Capabilities` — `AVA_SDK_EXPAND`'s `*` skips `ava.skills` so the expanded SDK reference does not render a second index; gateway only reuses `ava.composer_commands.discover_commands` for `/`-autocomplete, does not inject prompt
