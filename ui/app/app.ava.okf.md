...
`Cargo.toml` is version `0.4.2`; release tags (`app-v*`, `android-v*`) must match
it exactly — the release workflows fail on mismatch — so a release starts by
bumping `Cargo.toml`. `.github/workflows/ci-app.yml` path-filters
