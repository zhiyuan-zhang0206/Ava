...
served through the same-origin `/grafana` proxy, so `frame-src` permits only
the frontend origin. Production permits `'unsafe-inline'` only through CSP3's
`style-src-attr`, because dynamic layout attributes require it; `script-src`
remains nonce-only with no unsafe directive.
