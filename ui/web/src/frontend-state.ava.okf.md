...
description: Three mechanisms — TanStack Query (server data + persistent preferences in DB user_settings, #657) / two Zustand stores (volatile UI + SSE timeline) / localStorage (4 per-device selections); per-thread timeline caching.
tags:
- frontend
---
...
- A buffered/in-flight event for a just-parked thread still folds into its bucket (R3); inactive agents are not continuously streamed — switch-back restores parked state (R2), then fetch-on-enter reconciles the gap.
