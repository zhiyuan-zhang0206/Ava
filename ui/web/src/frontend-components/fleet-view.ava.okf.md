...
- **GraphView** (`graph-view.tsx`) — force-directed relationship graph: spawn/fork/resurrect lineage + attenuated message traffic. Nodes use the same public lifecycle statuses as the sidebar; when intermediate parent agents terminate, live descendants re-parent to their nearest live ancestor to preserve lineage springs. It mounts the live + terminated roster queries itself (same caches home's `useAgents` seeds): the graph payload is live-only, so the terminated roster is the lineage walk's input for re-parenting through terminated intermediates even on a direct `/fleet` load.
...
- **ForceGraph** (`force-graph.tsx`) — canvas shared by GraphView and TaskGraph. Its d3 zoom/pan behavior, transform state, reset, and box-fit math come from `lib/use-svg-zoom-pan.ts`, also used by the memory graph; each canvas retains its own layout timing and focus-neighbor bounds.
