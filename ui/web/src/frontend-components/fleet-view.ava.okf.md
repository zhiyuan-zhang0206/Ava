...
- **GraphView** (`graph-view.tsx`) — force-directed relationship graph: spawn/fork/resurrect lineage + attenuated message traffic. Nodes use the same public lifecycle statuses as the sidebar; degree-0 orphan nodes are filtered out so only connected agents are graphed.
