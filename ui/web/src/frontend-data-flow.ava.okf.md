...
- **Cluster update takeover + reconnect**: global `cluster_update_started` makes `AppConnectionBanner` seed the `["cluster-status"]` cache with the orchestration and set `clusterUpdating`, so `AuthGuard` immediately shows full-screen `UpdatingPage` for every auth status. `use-cluster-health` polls `/api/cluster/status` (5s while updating; bypasses paused 503), mirrors the real orchestration state, distinguishes stranded pause, and on the real paused true→false finish edge clears the takeover, reconnects SSE, and refetches agents without waiting for the 45s watchdog.
