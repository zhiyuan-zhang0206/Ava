...
| [Cluster consistency: commit-level pinning](infra/commit-pinned-cluster.md) | Increments A + B built. The remaining hard fail-fast enforcement is **overtaken** by the fail-fast-vs-reconcile decision and the [unified cluster lifecycle](infra/unified-cluster-lifecycle.md), and needs re-litigating before it is built |
| [Ops module](../ops/ops-module.md) | Spec / Status built. Native maintenance drain and prepared release requests own lifecycle; pg-backup is a supervised scheduler service |
