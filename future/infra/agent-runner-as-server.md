...
| Wedged-state recovery (reaper force-kill of lease zombies) | `asyncio.Task.cancel()` on the turn task, escalating to the Phase 2 worker restart (or Phase 1 runner restart) for a truly uncancellable turn. The zombie *class* shrinks: no process can silently die while `idling`, because idle has no process. |
