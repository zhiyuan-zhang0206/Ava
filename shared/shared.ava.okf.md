...
- **Structured logging + metrics** (`shared/log.py`, `shared/metrics/report.py`): one loguru singleton over stderr / JSONL file / the unified event pipeline, plus the metrics core (`shared/metrics_aggregate.py`) — the digest behind `/api/metrics` reads Loki aggregates via `gateway.loki_events`. The unified emitter (`shared/telemetry/emitter.py`) mirrors events to JSONL, projects compact numeric observations through `shared/observed_metrics.py`, and exports to OTLP ([[shared/telemetry/otlp/telemetry-otlp/telemetry-otlp.ava.okf.md|OTLP exporter]] — Loki logs + Prometheus metrics). The Postgres `events` archive was dropped with the cleanup (task #1281/#1823); the projected observations contain no log or checkpoint bodies. Projection is best-effort, deduplicated by source event identity, and transactionally updates daily sums. Upstream loss remains possible, so ingestion freshness and successful replay never certify completeness. `shared/audit_events.py` is the audit entry point.
