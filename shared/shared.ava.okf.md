...
- **Agent cross-process contract** (`shared/agents/contract.py`): AgentStatus enum, exception hierarchy, wire error protocol (HTTP error transmission between gateway ↔ agent SDK).
- **Message-level contract** (`shared/message_kwargs.py`): supplements the above — `AvaMsgType`/`AvaMessageKwargs` (strongly-typed view of `ava_*` metadata in `additional_kwargs`). Gateway message inserts also carry nullable server-owned audit facts — [[shared/agents/messages/inbound-provenance.ava.okf.md]].
- **Structured logging + metrics** (`shared/log.py`, `shared/metrics/report.py`): one loguru singleton over stderr / JSONL file / the unified event pipeline, plus the metrics core (`shared/metrics/metrics_aggregate.py`) — the digest behind `/api/metrics` reads Loki aggregates via `gateway.loki_events`. The unified emitter (`shared/telemetry/emitter.py`) mirrors events to JSONL, projects compact numeric observations through `shared/metrics/observed_metrics.py`, and exports to OTLP ([[shared/telemetry/otlp/telemetry-otlp/telemetry-otlp.ava.okf.md|OTLP exporter]] — Loki logs + Prometheus metrics). The Postgres `events` archive was dropped with the cleanup (task #1281/#1823); the projected observations contain no log or checkpoint bodies. Projection is best-effort, deduplicated by source event identity, and transactionally updates daily sums. Upstream loss remains possible, so ingestion freshness and successful replay never certify completeness. `shared/audit_events.py` is the audit entry point.
...
- **HTTP/RPC API contracts** (`shared/api_contracts/`): gateway↔cli HTTP response types (`ConfigFieldView`/`ConfigSectionView` etc.), route idempotency/pause declarations (`contracts.py`), the runner RPC envelope (`op_envelope.py`), and stdlib-only durable JSON field checks (`strict_decode.py`).
- **Converge status files** (`shared/host/converge/_status_file.py`): shared JSON/file framing for screen capture and accessibility status; each caller owns its state and notice text.
