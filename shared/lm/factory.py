...
def model_supports_vision(model: str) -> bool:
    ...
    their builder owns the wire shape, and models generated by a plugin do not
    need to repeat the endpoint capability in every `ModelSpec`. A model
    matching no core or plugin prefix is text-only by default — the message
    endpoint 422s images up front rather than letting the provider 400 mid-turn.
    """
