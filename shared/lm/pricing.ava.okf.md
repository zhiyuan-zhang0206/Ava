...
- **DeepSeek's 2026-09-10 V4.1-Flash release cut Flash prices to `$0.003/$0.006` cache hit, `$0.15/$0.30` cache miss, `$0.60/$1.20` output (off-peak/peak)** from 2026-09-09T16:00:00Z — the Beijing-midnight boundary for the Change Log date the page publishes in place of an instant — and retired the V4 Flash names: `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` remain provider-side compat ids (served as V4.1-Flash, billed at the Flash price); Ava resolves both to `deepseek-flash` before provider construction — name only `deepseek-flash` (user report 2026-09-20). V4 Pro routes to V4.1 Flash from 2026-09-14T04:00:00Z; the archive records that succession as the entry's final period. The page scopes peak hours Monday-Friday while the archive's UTC windows recur daily, so weekends bill at peak rates — a known overestimate.
- **Xiaomi's MiMo V2.6 launch prices** are `$0.435/$0.0036/$0.87` for `mimo-v2.6-pro` and `$4.35/$0.036/$8.70` for `mimo-v2.6-pro-ultraspeed` (cache miss/cache hit/output USD per 1M; [official pricing](https://mimo.mi.com/docs/price/pay-as-you-go), checked 2026-09-22). `mimo-v2.5-pro` stays config-valid at identical rates but is display-superseded by V2.6 Pro; withdrawn `mimo-v2.5-pro-ultraspeed` retains its historical price while persisted configurations resolve to V2.6 Pro UltraSpeed before provider construction.
