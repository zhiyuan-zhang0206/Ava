...
- **qwen3.8-flash's USD is sourced from QwenCloud** (`$0.16 / $0.016 / $0.47`): the Model Studio EN page (Beijing USD column) was not yet published at onboarding (2026-08-27). QwenCloud's USD for qwen3.8-max equals Model Studio's Singapore column, so this entry may overstate Beijing cost until the model page lands — the conservative direction (see the module docstring).
