...
class AgentPromptSettings(EnvSettings):
    ...
    skill_match_enabled: bool = Field(
        default=False,
        alias="AVA_SKILL_MATCH_ENABLED",
        description=(
            "Semantically match each expanded chat inbound against this agent's "
            "Capabilities skill index and inject a separate load hint for strong "
            "matches. Disabled by default; failures are ignored so matching never "
            "blocks the user message."
        ),
        json_schema_extra={
            "restart_required": "agent",
            "writable": True,
    ...
    skill_match_top_k: int = Field(
        default=3,
        alias="AVA_SKILL_MATCH_TOP_K",
        ge=1,
        description="Maximum number of above-threshold skills named in one semantic match hint.",
        json_schema_extra={
            "restart_required": "agent",
            "writable": True,
            "sensitive": False,
    ...
    skill_match_min_score: float = Field(
        default=0.35,
        alias="AVA_SKILL_MATCH_MIN_SCORE",
        ge=0.0,
        le=1.0,
        description=(
            "Minimum cosine similarity required to inject a semantic skill hint. "
            "Higher values trade recall for fewer irrelevant hints."
        ),
        json_schema_extra={
            "restart_required": "agent",
            "writable": True,
    ...
    skill_match_budget_ms: int = Field(
        default=300,
        alias="AVA_SKILL_MATCH_BUDGET_MS",
        ge=1,
        le=300,
        description=(
            "Hard wall-clock budget in milliseconds for embedding one chat inbound "
            "during semantic skill matching. Timeout skips the hint and delivers the "
