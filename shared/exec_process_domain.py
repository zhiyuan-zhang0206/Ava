    ...
    def _confirm_closure(self, deadline: float) -> None:
        ...
        while True:
            try:
                ...
                # XNU returns EPERM for a group containing only the retained
                # zombie. Confirm both facts after the attempted kernel signal;
                # this is trusted-tool cleanup, not an arbitrary-code fence.
                self._require_unreaped()
                if (
                    sys.platform != "darwin"
