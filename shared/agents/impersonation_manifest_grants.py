...
def grant_manifest_runner_access(conn: psycopg.Connection, runner_role: str) -> None:
    ...
    conn.execute(
        sql.SQL(
            "GRANT UPDATE (status,expires_at,rejection_reason,summary_inbound_id,summary,"
            ...
            "next_entry,event_delivery_pending_reason) ON agent_impersonations TO {}"
        ).format(role)
    )
    for table in (
    ...
    for signature in (
        ...
        "public.certify_impersonation_event_delivery(uuid)",
    ):
