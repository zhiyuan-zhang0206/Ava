...
- **stranded hold record** (`stranded_hold_since` / `stranded_hold_reason`) — the pause controller's record of a maintenance hold whose owner died, the signature of a failed updater leg (task #3132): `since` stamped once per episode, `reason` the latest reading; declared past a ten-minute bound, cleared only on a *decided* reading — an unreadable round of either reader stands. Read by the gateway alarm (`update failed: host left held`) and every roster surface, since the held host's probe is usually down.
...
- `mark_stranded_hold` / `clear_stranded_hold` — declare / decided clear of the stranded-hold record
