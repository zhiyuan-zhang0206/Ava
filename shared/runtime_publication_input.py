...
def _published_unit(expected: ExpectedUnitWriters, prepared_receipt_digest: str) -> PublishedUnit:
    """Keep observer identity distinct from the complete prepared receipt."""
...
def resolve_runtime_publication_input() -> RuntimePublicationInput | None:
    ...
    receipt_path = home / "run" / f"release-inventory-{selector.prepared_receipt_digest}.json"
    ...
    if prepared_receipt_digest != selector.prepared_receipt_digest:
    ...
    return RuntimePublicationInput(
        _published_unit(expected, prepared_receipt_digest),
        selector,
...
def revalidate_runtime_publication_input(value: RuntimePublicationInput) -> None:
    ...
    receipt = home / "run" / f"release-inventory-{value.actual.prepared_receipt_digest}.json"
    if receipt.resolve(strict=True) != receipt:
        raise ReleaseRejectedError("runtime receipt path changed")
    if hashlib.sha256(regular_bytes(receipt)).hexdigest() != value.actual.prepared_receipt_digest:
