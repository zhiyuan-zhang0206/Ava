...
### Two key mechanisms
- The day-stamped JSONL mirror (`$AVA_HOME/logs/events-YYYYMMDD.jsonl`) persists the same stable surrogate `id` as the Loki read path, so id-based deduplication and duplicate detection work on mirror rows.
