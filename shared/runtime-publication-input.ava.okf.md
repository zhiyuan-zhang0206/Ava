...
artifact_digest, manifest_digest and prepared_receipt_digest. The old exact
...
Its `prepared_receipt_digest` stays distinct from the narrower observer tuple's
`inventory_digest`; its bytes, complete service roster and expected image/home
are validated. The loaded package's baseline file supplies an independent integrity comparison for
