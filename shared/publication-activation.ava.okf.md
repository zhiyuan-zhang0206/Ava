...
description: Retained SQL publication authority fences migration, selector and service readback evidence.
---

# Pending service activation

`managed_writer_activation.py` is the retained short-transaction
publication authority API, not an RPC, controller, liveness scanner or agent-birth permit.
...
The retired CLI/ops updater journal, collection, activation and completion
producers are absent. These shared storage APIs remain because runtime admission
still consumes their current/pending fences. No compatibility producer translates
old journals into prepared release authority, and deleting the command graph does
not clear retained pending state.

No production migration, normal service activation or protocol advertisement is
performed by importing or testing these helpers.
