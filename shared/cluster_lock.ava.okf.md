...
- `claim_pending_recovery_lease` (`shared/cluster_pending_recovery.py`, split out for the file-size budget) — the pending-publication recovery protocol's own takeover CAS: only an executing rollout whose journaled `pending.operation` still equals the exact subdocument the caller inspected — and whose provably dead lease identity still matches — is replaced; `target_sha` is preserved.
...
- `read_update_lease` → `DeployLease` (holder / held_for_s / expires_in_s / kind / settle fields; `is_settle_hold` and `awaits` are the discriminating reads) — the row itself, for consumers that must explain the hold or tell an executing rollout apart from a settle hold; `update_lock_holder()` is the bare "is it held".
- `update_lock_refusal_detail()` (with `_acquire_refusal_reason` behind the acquire warning) — the operator-facing reading of a refused acquire, naming its own next step: a live holder (wait it out; a coexisting durable pending publication is named as its follow-up, `ava cluster recover-pending`), a pending-only durable publication (its checked recovery, `ava cluster recover-pending`), or a racing acquire.
...
- **Settle holds** — `settle_update_lock(holder, hosts)`: an orchestration that ends with hosts still converging keeps the lease held on its own TTL (`SETTLE_TTL_S` = `NO_PROGRESS_TIMEOUT_S`) instead of releasing, so a second deploy cannot start into a half-transitioned cluster. The settle fact is written as structured columns in one statement: the `settle_hosts` array (the truth every reader branches on), `settle_note` (the human-readable sentence), and `settle_started_at` (the telemetry anchor). `release_settle_hold` and `renew_update_lock` are scoped on `settle_hosts` (`IS NOT NULL` / `IS NULL`), so a convergence check can only ever clear a settle hold and a renewal can never re-arm one — the legacy single-column `note` is gone.
...
- `acquire_update_lock` / `renew_update_lock` / `release_update_lock` / `settle_update_lock` / `release_settle_hold` / `read_update_lease` / `update_lock_holder` / `update_lock_refusal_detail` — `shared/cluster_lock.py`
- `claim_pending_recovery_lease` — `shared/cluster_pending_recovery.py` (the pending-publication takeover CAS)
- `ops/publication_recovery.py` — the checked recovery seat (`run_pending_publication_recovery` / `pending_publication_recovery_op`; storage-level `claim_abandoned_pending_lease` / `complete_pending_publication_recovery`), operator entrance `ava cluster recover-pending`
