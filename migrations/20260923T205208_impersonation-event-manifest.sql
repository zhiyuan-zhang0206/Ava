    ...
    IF EXISTS (
        WITH expected AS (
            SELECT event_key, min(event_kind) AS event_kind, min(line_sha256) AS line_sha256 FROM (
                SELECT event_key, event_kind, line_sha256 FROM public.agent_impersonation_event_participant_items
                WHERE lease_id = p_lease_id
                UNION ALL
                SELECT event_key, event_kind, line_sha256 FROM public.agent_impersonation_event_expected_items
                WHERE lease_id = p_lease_id
            ) all_expected GROUP BY event_key
        ), actual AS (
            SELECT event_key, kind AS event_kind, payload->>'line_sha256' AS line_sha256
            FROM public.agent_impersonation_entries
            WHERE lease_id = p_lease_id AND kind IN ('sdk_call', 'api_event')
        )
        (SELECT event_key, event_kind, line_sha256 FROM expected
         EXCEPT SELECT event_key, event_kind, line_sha256 FROM actual)
        UNION ALL
        (SELECT event_key, event_kind, line_sha256 FROM actual
         EXCEPT SELECT event_key, event_kind, line_sha256 FROM expected)
    ) THEN
