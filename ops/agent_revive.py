...
The hibernate/boot half of the wake family (Task #1999 split out of
`ops/agent_wake.py`, which keeps resurrect / respawn and the resurrect claim
machinery): `swap_in_agent` wakes a `hibernating` row and `revive_agent` wakes
a `running`/`idling` row behind a dead pid. Both share the same shape as
resurrect — a CAS into unclaimed `idling`, then a relaunch attached to the same
`agent_id` (LangGraph's checkpointer restores the history, so the process
resumes rather than starts over) — and both are deliberately INVISIBLE to the
agent: no lifecycle inbound, no marker, so a woken agent cannot tell it was
ever swapped out or revived.
...
The caller MUST run both on the agent's home machine (`agents_meta.machine`) —
launching on any other host trips the boot placement gate and crash-loops
(agent 1513 incident). The hibernation controller and the restarter reaper
satisfy this by only scanning `machine = local`.
"""
...
def swap_in_agent(agent_id: int) -> bool:
    ...
    if runner_mode.is_hosted():
        # Hosted: the row flips to 'idling' and the dispatcher's wake (the one
        # that carried the inbound triggering this swap-in, re-published by the
        # delivery watchdog if it was lost) materializes the turn. No process,
        # no stale-session kill. The explicit wake covers the reaper-driven
        # flip where the original wake predated the status flip.
        publish_inbound_wake(agent_id, "0")
        logger.info(
            "agent {agent_id} swapped in from hibernation in hosted mode — "
