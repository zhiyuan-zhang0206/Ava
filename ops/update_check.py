...
def update_check() -> UpdateCheck:
    ...
    release tag in `releases` mode — then compares the running-commit bookmark
    (recorded at gateway start) against it, not HEAD, so a manual `git pull`
    between start and the preflight cannot hide code changes. Falls back to
    HEAD when the bookmark has never been recorded. Pure read-only — no pull,
    no working-tree mutation — so the UI can poll it to keep the Update
    button's state ("no updates" vs "N commits, restarts backend") fresh
    without side effects. Mirrors the classification `ava cluster update` runs
    internally before deciding what to restart.
    """
    ...
    if settings.general.track_mode == "releases":
        ...
        if tag is None:
            ...
            return UpdateCheck(behind=0, frontend_changed=False, backend_changed=False)
        target_ref = tag.name
    else:
        target_ref = f"origin/{settings.general.track_branch}"
    baseline = _get_running_sha() or "HEAD"
    ...
    return UpdateCheck(behind=behind, frontend_changed=frontend, backend_changed=backend)
