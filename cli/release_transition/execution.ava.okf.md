...
description: Phase-by-phase release effects, recovery direction, root start through the home boot owner and fresh readiness before resume.
...
`local.py` owns those release effects. `root_service.py` uses the existing home
boot unit, never a second application supervisor. Its one-operation start
action runs `stage.py` in that unit; the new root therefore survives the finite
executor's exit. Readiness checks every selected service plus native root
identity. Resuming checks fresh readiness again, including after an interrupted
executor or a previously written resume receipt. A durable serving marker is
not a substitute for a live observation.
...
steady pinned-image boot action. `shared/release_operation.py` blocks unrelated
