"""Explicit operator validation for a retained image, before settings or source.

Image and expected inventory are not a bootable rollback or all-writer closure.
This entry validates the exact local request, then refuses before dispatch.
No publication, writer closure, migration, selector, service start/readback, or
finalization effect is implemented or authorized here.
"""
...
def _image(unit: PublishedUnit, schema_digest: str) -> tuple[VerifiedRelease, PreparationReceipt]:
    ...
    raw = regular_bytes(home / "run" / f"release-inventory-{unit.prepared_receipt_digest}.json")
    if hashlib.sha256(raw).hexdigest() != unit.prepared_receipt_digest:
    ...
    ) or receipt.inventory_digest != unit.inventory_digest:
        raise ReleaseRejectedError("prepared inventory belongs to another unit/image")
    if expected.unit().inventory_digest != unit.inventory_digest:
        raise ReleaseRejectedError("prepared inventory belongs to another unit/image")
...
def prepare_operator_input(path: Path) -> PreparedOperatorInput:
    ...
    if request.valid_until <= datetime.now(UTC):
        raise ReleaseRejectedError("prepared entry deadline expired during verification")
...
def run_prepared_update(args: argparse.Namespace) -> int:
    """Validate the retained request, then refuse before any shared-state effect.

    A production runner has read-only access to deployment publication state.
    Until an authenticated gateway adoption boundary and exact pre-stop abort
    exist, treating local validation as a writable participant acknowledgement
    would either fail on runners or strand the cluster in durable pending state.
    """
    if (
        not args.local
        ...
        return 2
    try:
        prepare_operator_input(Path(args.prepared))
        sys.stderr.write(
            "prepared input verified, but authenticated dispatch and exact pre-stop abort "
            "are not implemented; no operation was started\n"
        )
        return 2
    except (
        OSError,
