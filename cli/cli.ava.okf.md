...
| `ava cluster update` | capability-dispatched: gateway-capable orchestrates the cluster; pure agent-runner self-updates. Every sync temporarily makes a read-only editable `.pth` owner-writable and restores its exact mode before continuing |
| `ava converge` | replays idempotent host wiring (prod editable-`.pth` target/symlink/PATH/dirs/plugin images), usually via `ava start`; it never touches the memory pool |
