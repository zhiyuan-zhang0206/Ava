...
| `ava converge` | replays idempotent host wiring (prod editable-install + site-packages protection/symlink/PATH/dirs/plugin images), usually via `ava start`; it never touches the memory pool |
