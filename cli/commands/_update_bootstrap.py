...
from cli.commands._release_inventory import _regular_bytes, revalidate_prepared_inventory
...
def prepare_bootstrap_hop(request_path: Path) -> PreparedBootstrapHop:  # noqa: PLR0915 — ordered read-only admission before any updater effect.
    ...
    if journal is None:
        ...
        raw = json.loads(_regular_bytes(receipt))
...
def _read_recovery(request: Path, receipt: Path) -> tuple[str | None, BootstrapJournal | None]:
    ...
    journal = BootstrapJournal.model_validate_json(json.dumps(raw["bootstrap_hop"]))
    if (
...
def _journal(plan: PreparedBootstrapHop, generation: str, stage: str, cron: bytes) -> None:
    """Extend the existing updater handoff; never create a second recovery journal."""
    with file_lock(updater_handoff.lock_path(), timeout_s=5):
        ...
        if (
            ...
            or snapshot.owner_create_time != psutil.Process().create_time()
        ):
...
def _recorded_observer(plan: PreparedBootstrapHop) -> tuple[ExpectedProcess, str]:
    ...
    choices = {
        "A": bootstrap_command(plan.recovery_image, Path(plan.request.recovery_context)),
        "B": bootstrap_command(plan.image, Path(plan.request.candidate_context)),
    }
    matching = [kind for kind, argv in choices.items() if command == ["exec", *argv]]
    if len(matching) != 1 or record["cwd"] != plan.candidate.expected.home:
...
def _execute_bootstrap_hop(plan: PreparedBootstrapHop, generation: str) -> int:
    ...
    try:
        ...
        process, kind = _recorded_observer(plan)
        raw = json.loads(_regular_bytes(updater_handoff.state_path()))
        if raw["bootstrap_hop"]["stage"] == "candidate_starting" and (
            kind != "B" or process == plan.old_session.process
        ):
            raise ReleaseRejectedError(
