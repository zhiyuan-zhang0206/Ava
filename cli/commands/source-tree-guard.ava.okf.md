...
- **Repair** — `_converge` registers the "source tree reset + clean"
  ...
  whitelist, at every `ava start` / `ava cluster update`. Repairs print a
  warning and emit the registered anomaly event `source_tree_reset` with the
  reset range, cleaned paths, and kept whitelisted paths.
