"""One root-owned application service tree for start, stop and recovery.

macOS roots belong to the signed permissions helper. Linux roots use the
native POSIX adapter. Unsupported adapters refuse before spawning; ordinary
services never fall back to named sessions. Terminal sessions are separate.
"""
...
def complete_boot_start() -> None:
    ...
    if sys.platform != "linux":
        return
    from services.ava_root.client import native_identity
    from shared.os_boot_unit import in_boot_unit, notify_root_ready
    ...
    notify_root_ready(home, native_identity(snapshot["root"]))
...
def _spawn_direct(
    run_dir: Path, repo: Path, manifests: Path, env: dict[str, str]
...
def _seed_via_helper(run_dir: Path, repo: Path, manifests: Path, env: dict[str, str]) -> None:
...
def _bring_up_root(
    run_dir: Path, repo: Path, manifests: Path, client: Any, env: dict[str, str]
) -> dict[str, Any]:
    ...
    if _helper_spawn_committed():
        ...
        _seed_via_helper(run_dir, repo, manifests, env)
        return _await_root_status(client, run_dir)
    proc = _spawn_direct(run_dir, repo, manifests, env)
...
def _stop_root_process(
    ...
    if _helper_spawn_committed():
        ...
        if OwnedProcess.capture(process) != identity:
            raise _RootDriverError("root identity changed before stop")
        process.terminate()
    deadline = time.monotonic() + timeout_s
    while time.monotonic() < deadline:
        alive = identity.live()
...
def admit_live_start(
    ...
    if status is None:
        ...
        return False
    _require_root_owner(status)
    manifest = _tree_manifest(roster, repo, roles=roles)
    from shared.paths import ava_home

    manifest["launch_digest"] = launch_digest(repo, _root_child_env(), home=ava_home())
...
def _ensure_root_service_tree(
    ...
    try:
        ...
        if status is None:
            ...
            status = _bring_up_root(run_dir, repo, manifests, client, env)
            _require_root_owner(status)
            _require_same_generation(manifest, status, roster, reconcile=reconcile)
...
# line per lifecycle phase. Calls use the cli.commands seam shared by callers
...
def _next_gone_streak(
    ...
    return {
        spec.session: previous.get(spec.session, 0) + 1 if _unit_gone(units.get(spec.session)) else 0
        for spec in specs
    }
