...
Every import here stays in the light cli closure (the `_managed_writer_hop`
discipline): this module is reached only through the wiring's method-local
import, so the ops RPC stack, the database and the activation module it
borrows all load beside the call, never at `import cli.commands` time.
"""
...
from cli.commands._managed_writer_hop import ContinueUnitInput, ManagedWriterPhaseInput
from ops.cluster_session import _UPDATER_SERVICE
from ops.rpc_bootstrap_hop import BootstrapRecoveryReadResult
from ops.rpc_normal_continue import NormalContinueResult
