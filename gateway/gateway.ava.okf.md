...
- **Authentication and browser-origin policy**: auth follows the cluster secret. With a secret set, every `/api/*` request requires a session cookie or `Authorization: Bearer <cluster secret>` — except the `_AUTH_BYPASS_PATHS` set (`/api/health` for host probing, `/api/auth/login` / `check` / `logout` for the browser session flow); `auth_middleware_enabled=false` bypasses for testing/e2e. Cookie-authenticated POST/PUT/PATCH/DELETE requests with an `Origin` must match the exact CORS allowlist; bearer callers and clients without `Origin` are unaffected. The allowlist is explicit when configured, otherwise derived from the frontend port plus local and gateway hosts. Session-cookie `Secure` is explicit when configured, otherwise derived from `gateway_url`. An EMPTY `cluster_secret` is the no-auth posture (single-box default): the API serves unauthenticated, `/check` reports authenticated, and the gateway binds loopback only (`main()`). `/api/bootstrap` is used for agent-runner registration
