...
- **tasks** (`/api/tasks` GET + `/api/tasks/{id}` PATCH) — task registry read + partial update (no create; PATCH does not notify owner); GET defaults to a full compatibility row or serves SQL-truncated description/results previews with `fields=summary`; rows carry `priority` (`P0`..`P3`, validated, illegal 422)
