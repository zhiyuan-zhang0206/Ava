...
- **agents** (`/api/agents/*`) — spawn, terminate, resurrect, restart, compact, send_message, list, patch. `GET /api/agents` defaults to `scope=all&fields=full` for SDK/ops; roster consumers request SQL-projected `fields=summary` with live or terminated scope, while `ava agents ls` requests the three-field `fields=compact` projection. Summary keeps roster state and response-required notices, not checkpoint/probe internals or raw configuration; detail stays full. Plus `/api/cancel`, `/api/models`, `/api/agents/{id}/token-usage` (per-model soft/hard compact thresholds for the ContextMeter gauge), `/api/agents/{id}/context-breakdown` (#619: checkpoint messages bucketed by kind + system prompt by `#` sections; chars/4 normalized to the real input_tokens ratio; pure view in `gateway/context_breakdown.py`, one DB read), `/api/agents/{id}/exited` (exit finalize) — split by surface: CRUD + spawn in `agents.py`, lifecycle in `agents_lifecycle.py`, message/state reads in `agents_state.py`, cross-machine forward helpers in `agents_forward.py`
