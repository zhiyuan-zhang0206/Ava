...
- **run_timeline** (`/api/agents/{id}/run-timeline`) — event-driven run/turn view: reads the bounded Loki history for one agent, joins each `turn_end` to `llm_usage` by span ID, and returns either turn rows or caller-selected time buckets with lifecycle, compact, execution, idle, and failure markers. The default session is the latest observable lifecycle start through compact; missing lifecycle history falls back to the last 24 hours.
