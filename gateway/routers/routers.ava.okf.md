...
- **agent_inspect** (`/api/agents/{id}/inspect` + `/inspect/live` + `/neighbors` + `/inspect/metrics` + `/inspect/widgets`) — per-agent LLM cost/token/TPS + neighbor graph; plugin metric and inspector-widget surfaces — see [[gateway/routers/agent-inspect.ava.okf.md]]
