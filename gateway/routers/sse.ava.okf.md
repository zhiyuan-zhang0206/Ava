...
### throttled_event_stream (full batch throttled)
- Pushes **all events from all agents** (no agent_id, no role filtering), frontend filters internally.
- Drain Redis messages into a batch within a flush window, flush at a fixed rate: default `settings.sse_throttle_rate = 25/sec`.
...
| `/api/system/all` | throttled_event_stream | none (frontend filters) | system.py |
