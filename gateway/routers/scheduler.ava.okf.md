...
| `error` | crash loop breaker tripped (see below, **terminal**) | **no auto restart** (same skip as `completed`)—breaker state lives in DB, not in-memory `_backoff`, so **gateway restart won't restart it**; recover via `start`/`restart` manually (the existing API resets status + clears backoff) |
...
- [[db.ava.okf.md]] — reads/writes the `schedules` table
- `shared.cluster.session_name()` — generates the session name `ava-schedule-<id>`
- [[services/agent_runner_side/restarter.ava.okf.md]] — similar session management pattern, but schedules do tasks rather than guarding agents
