...
type: doc
title: Alerts Router
description: "POST /api/alerts + GET /api/alerts + GET /api/alerts/stream + PATCH /api/alerts/read — the system→human alert store (alerts), the alert-section history list, the SSE live tail, mark-as-read, and the IM notification fan-out (Task #1224)."
tags:
- gateway
- alerts
---
...
store/IM pipeline: one row + one IM notification per transition. The
...
(unique constraint, migration `20260813T042527_alerts`). Alertmanager
re-sends the same instance every group_interval while firing and once more
on resolution; the upsert updates the row instead of duplicating it.
