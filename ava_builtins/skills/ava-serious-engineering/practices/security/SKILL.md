...
- **Least privilege, everywhere, by construction**:Every process, role, credential, and network binding carries the minimum capability its job requires — so a compromise is contained by the boundary it happens in. — **Why**:Blast radius scales with privilege; the Ava data plane already embodies the pattern: the Postgres role is NOSUPERUSER owning only its own database, Postgres and PgBouncer bind loopback plus the reachable host (never all interfaces), Redis binds loopback-only, and a split deployment needs reachability *and* the cluster secret. — **How**:Per-component credentials; scoped API tokens; at review, verify no role or credential can do more than its caller needs; keep network bindings as narrow as the deployment allows.
