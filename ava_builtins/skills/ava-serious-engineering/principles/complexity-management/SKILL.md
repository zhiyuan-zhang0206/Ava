...
- **Locality is information hiding made observable — design for local reasoning**:A codebase has good locality when a reader can establish that a change is correct by reading only the module being changed plus the public contracts of its neighbors — the way you debug a network through the protocol's contract without reading the TCP stack. — **Why**:Change amplification, cognitive load, and unknown unknowns all shrink when both the edit and the reasoning stay local; for an AI agent, locality is the number of tokens a correct change costs (see `ai-era/guidance.md`, Context Budgeting). File length and directory width are only proxies: fifty small files that reach into each other's internals have worse locality than one deep module. — **How**:Three legs, all required. (1) One narrow door per package: callers import only public names through the package interface, and underscore-private names stay private — enforced by a lint, because an unenforced convention erodes. (2) One owner per design decision: when the same decision (a connection posture, an identity key, a wire format) is made at many call sites, route the sites through its owner rather than patching each one. (3) Contracts verified at the door: tests pinned to the public surface and fail-fast errors phrased in the contract's vocabulary, so trusting the door is justified. Measure locality by module spread per change and by cross-package co-change over history (a pair of modules that keeps changing together names a leaked decision); use those numbers as an index for sweeps, never as a merge gate — a gate on PR width teaches authors to split PRs, not to fix leaks.
...
- [ ] **MUST** Does the change import a private name from outside the package that owns it, or re-make a decision that already has an owner? Go through the door or the owner instead; if the name is genuinely needed outside, promote it into the contract deliberately.
- [ ] **SHOULD** If the change spans three or more modules, can you name the design decision that leaked across them — and either close the leak now or record it as debt?
- [ ] **SHOULD** If a bug fix must change a public contract, is it treated as a design bug — the contract change made explicit and the intent re-aligned — rather than slipped in as an internal fix?
