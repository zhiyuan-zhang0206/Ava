...
Go through seven dimensions, judge each as pass / partial-concern, each finding must include **file path + line number** (no location, no finding): ① AGENTS.md compliance (highest value — natural language, unmechanizable; includes doc sync discipline: if API endpoints / schema fields / glossary / roadmap items are changed, are docs updated in the same PR) ② PR description quality ③ doc sync ④ security patterns ⑤ test coverage judgment ⑥ architectural consistency ⑦ numeric limits (config-with-reason or a marked exception per `conventions/numeric-limits.md`; a naked limit is a must-fix). When a 🔴 must-fix item is found, additionally escalate via `ava.ui.notify` (P1).
