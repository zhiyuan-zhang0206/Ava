...
def backfill(days: int, week: str | None = None) -> Path:
    ...
    while d <= end_date:
        mf = MIRROR_DIR / f"events-{d.strftime('%Y%m%d')}.jsonl"
        if mf.exists():
            with mf.open() as fh:
                for line in fh:
                    if not line.strip():
...
if __name__ == "__main__":
    backfill(int(sys.argv[1]), sys.argv[2] if len(sys.argv) > 2 else None)
