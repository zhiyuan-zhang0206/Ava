...
- **Remind**: SELECT `status='in_progress' AND owner IS NOT NULL AND NOT is_root` and `now()-updated_at > remind_interval_seconds`, `ORDER BY priority, id` (P0 first); deliver one per-owner `chat` digest through a direct `inbound_messages` INSERT (`kind='chat'`, `source='system'`), publish an owner invalidation hint after commit, then best-effort Redis-wake it. The digest copy is imperative (user ruling 2026-08-29): each line orders the owner to report current status and advance the next step, and to state why and raise `remind_interval_seconds` when it cannot advance — an explicit wait instead of silent idling. On success `last_reminded_at=now()`, `reminder_count += 1`, and telemetry records `task_reminder_digest`. `last_reminded_at` gates the backoff (`AVA_TASK_REMINDER_BACKOFF_SECONDS`, default 3600s)—at most one reminder per backoff within the same overdue window (so the count can climb to the escalate threshold in one window).
- **Escalate**: fires once when `reminder_count` reaches `AVA_TASK_ESCALATE_N` (default 3). Two paths: **with delegator** (parent owner non-null) → per-owner digest chat to the parent owner; **without delegator** → INSERT a require_response `agent_notices` on the stuck owner (`task_id`=that task, `priority` inherited, "stuck, please reassign or cancel"). The notice is attached to the agent but **audience is human**—the notice read surface supplies its content to the human queue and `GET /api/notices/escalations` supplies the operator's complete review context. Delegator path sends at `== threshold` once; the user path uses `>=` (an already-open notice **skips the round and retries next sweep**, so slipping past the threshold never permanently misses the window), and the sent notice itself is the idempotency marker, keeping "at most one open notice per agent". Both paths record `task_escalation` telemetry.
- **No resurrection for terminated owners**: direct delivery leaves their system `inbound_messages` rows unclaimed and only attempts the best-effort Redis wake; it never resurrects the terminated owner. The task instead escalates for reassignment or cancellation (2026-08-22 user ruling).
...
## Key Dependencies
- [[db.ava.okf.md]] — reads `agent_tasks` (SELECT overdue tasks, JOIN parent for `parent_owner` + task `priority`) + writes `last_reminded_at`/`reminder_count` counts + direct `inbound_messages` INSERT (`kind='chat'`, `source='system'`) + user escalation path writes `agent_notices` (require_response, then `publish_agent_updated_sync` invalidates the live view after commit)
- [[../../../../gateway/gateway.ava.okf.md]] — `GET /api/notices/escalations` is the operator query surface for open task escalations; direct delivery refreshes the owner badge and only best-effort wakes Redis, never auto-resurrects
