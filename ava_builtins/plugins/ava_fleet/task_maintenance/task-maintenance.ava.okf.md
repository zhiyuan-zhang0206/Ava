...
## Key Dependencies
- [[db.ava.okf.md]] — reads `agent_tasks` (SELECT overdue tasks, JOIN parent for `parent_owner` + task `priority`) + writes `last_reminded_at`/`reminder_count` counts + direct `inbound_messages` INSERT (`kind='chat'`, `source='system'`) + user escalation path writes `agent_notices` (require_response, then `publish_agent_updated_sync` refreshes snapshot so the queue updates live)
