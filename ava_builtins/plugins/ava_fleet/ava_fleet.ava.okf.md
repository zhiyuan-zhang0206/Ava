...
- **Self** only injects `set_label` (`plugin.py` `register_namespace_member`) — other members of core ava.self are not affected by this plugin; **Notify**'s `ui.notify` family is entirely registered by `plugin.py`
