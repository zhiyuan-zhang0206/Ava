...
### `create(title, description, *, parent=None, owner=None, priority="P2", remind_interval_seconds=1800) -> Task`
...
- `remind_interval_seconds`: no-update duration after which the owner is reminded (default 30 min). **Cannot be disabled**—`None` falls back to default; cap 24h; out-of-range raises `ValueError`.
...
### `create_and_assign(title, description, *, preset="coder", label=None, config_overlay=None, parent=None, priority="P2", remind_interval_seconds=1800) -> (Task, int)`
