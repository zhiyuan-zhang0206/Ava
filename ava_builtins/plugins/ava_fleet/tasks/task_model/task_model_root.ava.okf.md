...
- **`owner IS NULL`** — the **only** task without an owner; `created_by='system'`, `status` is always `in_progress` (its permanent state, pinned by the `agent_tasks_root_status_in_progress` DB CHECK). This is why `Task.owner` and `Task.remind_interval_seconds` are `int | None`: reading the root task returns `None` for both.
