...
One of three values a task may hold (`CHECK` constraint): `"in_progress"`, `"done"`, `"cancelled"`. A task is born `"in_progress"` (DB default — the `"open"` state was dropped by user ruling 2026-08-29: creation starts the work immediately; the `"ongoing"` state was removed by user ruling 2026-09-15: it was never needed). The system root is not a status: it is pinned to `"in_progress"` by the `agent_tasks_root_status_in_progress` DB constraint and is immutable. See [[task_lifecycle.ava.okf.md|Task Lifecycle]].
