...
- `ava.tasks.create(title, description, *, parent=None) -> Task` — mint a task. The creating agent automatically becomes the owner. `title` is the one-line name seen in listings (unique among open/in_progress tasks); `description` is the full detail whoever works it reads. Pass `parent` to make it a subtask — split a big task by creating several children under it.
