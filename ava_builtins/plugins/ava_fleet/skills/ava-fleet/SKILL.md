...
The budget ceiling is enforced softly: a watcher meters, a human decides the hard stop. The spawner points a watcher (`ava.watcher.launch` / `ava.watcher.cron`) at `reference/usage.py`, which reports per-agent and total spend from the LGTM read side (the durable cost ledger + Loki — the PG `events` table was frozen at the LGTM cutover and later dropped):
