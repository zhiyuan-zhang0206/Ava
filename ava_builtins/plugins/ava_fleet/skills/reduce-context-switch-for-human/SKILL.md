...
How often it updates is agreed **at delegation time**, by delegator and executor, from the nature of the work — spanning per-PR, daily, per-milestone, or only on completion; two agents' cadences can differ by orders of magnitude. The framework fixes no interval or trigger; cadence and budget are negotiated per delegation, alongside the goal and boundaries.
