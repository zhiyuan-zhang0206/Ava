...
## Page Lifetime (TTL)
Every page this stack **creates** — an explicit `new_page`, or the auto-created page on a page-less first navigate — carries a hard TTL deadline (`AVA_CHROME_PAGE_DEFAULT_TTL_SECONDS`, 24h default), enforced by the expiry sweep (`services/browser/page_lifecycle.py:reap_expired_pages`) beside the dead/idle reapers. Activity never extends it; a page in use across days is bounded by its TTL (reclaimed within a sweep interval).
